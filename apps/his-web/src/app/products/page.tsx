...
import { ApiError, createProduct, listProducts, type CatalogCreateInput, type CatalogRecord, updateProduct } from '@/lib/api';
...
export default function ProductsPage() {
  ...
  return (
    <ListPageLayout>
      <PageHeader
        title="Produtos"
        description={`Catálogo operacional de produtos (${titleSuffix})`}
        actions={<Button variant="primary" onClick={() => setCreateOpen(true)}>Novo Produto</Button>}
      />
      ...
      {loading && !error && <LoadingState message="Carregando produtos..." />}

      {!loading && !error && !isQueryTooShort && data.length === 0 && (
        <EmptyState
          title="Nenhum produto encontrado"
          description={debouncedQuery ? `Sem resultados para "${debouncedQuery}"` : 'Cadastre o primeiro produto do catálogo.'}
          action={<Button variant="primary" onClick={() => setCreateOpen(true)}>Criar Produto</Button>}
        />
      )}
