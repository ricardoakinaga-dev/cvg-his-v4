...
export default async function ClinicalRecordPage({ params }: { params: { id: string } }) {
    if (!params?.id) {
        notFound();
    }

    try {
        const result = await getPatientSummary(params.id);
                    ...
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: px(12), marginBottom: px(4) }}>
                            ...
                            <span style={{
                                padding: `${px(2)} ${px(8)}`,
                                borderRadius: px(16),
                                background: theme.colors.primary + '1A', // 10% opacity primary
                        ...
                        <p style={{ color: theme.colors.textSecondary, fontSize: px(14), margin: 0 }}>
                            {patient.species && `${patient.species} • `}{patient.breed && `${patient.breed} • `}ID: {patient.id}
                        </p>
                    </div>
        ...
        console.error('Failed to load clinical record context:', err);
        return (
            <div style={{ padding: px(40), textAlign: 'center' }}>
                <h2 style={{ color: theme.colors.danger }}>Erro de Carregamento</h2>
                <p style={{ color: theme.colors.textSecondary }}>Não foi possível carregar o prontuário para este paciente.</p>
                <Link href="/patients">
