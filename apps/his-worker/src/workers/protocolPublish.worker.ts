...
import {
  PROTOCOL_PUBLISH_LOCK_TTL_SECONDS,
  acquireRedisLock,
  protocolPublishLockKey,
  releaseRedisLock
...
type ProtocolVersionStatus = 'draft' | 'publishing' | 'published' | 'failed';
...
function mapValidationPath(path: Array<string | number>): string {
  return path
...
async function publishVersion(input: ProtocolPublishJobData): Promise<ProtocolPublishJobResult> {
  ...
  try {
    ...
    const row = lockedVersionResult.rows[0] as Record<string, unknown>;
    ...
    if (versionStatus !== 'publishing' && versionStatus !== 'published') {
      throw new NonRetryablePublishError(
        `Protocol version is not publishable in worker (status=${versionStatus})`
      );
    }
    ...
    if (parsed.data.protocolId !== input.protocolId) {
      throw new NonRetryablePublishError(
        `Invalid protocol publish content_json: protocolId (${parsed.data.protocolId}) does not match job protocolId (${input.protocolId})`
      );
    }
    ...
    if (!snapshotId) {
      ...
      const snapshotRow = insertResult.rows[0] as Record<string, unknown>;
...
function logInfo(context: PublishLogContext, message: string, extra: Record<string, unknown> = {}): void {
  console.info(
    JSON.stringify({
      level: 'info',
      message,
...
function logWarn(context: PublishLogContext, message: string, extra: Record<string, unknown> = {}): void {
  console.warn(
    JSON.stringify({
      level: 'warn',
      message,
...
function logError(context: PublishLogContext, message: string, extra: Record<string, unknown> = {}): void {
  console.error(
    JSON.stringify({
      level: 'error',
      message,
...
export function createProtocolPublishWorker(
  connection: Redis,
  prefix: string
): Worker<ProtocolPublishJobData, ProtocolPublishJobResult, ProtocolPublishJobName> {
  return new Worker<ProtocolPublishJobData, ProtocolPublishJobResult, ProtocolPublishJobName>(
    PROTOCOL_PUBLISH_QUEUE_NAME,
    async (job) => {
      ...
      const lock = await acquireRedisLock(
        connection,
        protocolPublishLockKey(payload.versionId),
        PROTOCOL_PUBLISH_LOCK_TTL_SECONDS
      );
