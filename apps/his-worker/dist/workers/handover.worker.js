...
import { HANDOVER_BUILD_JOB_NAME, HANDOVER_BUILD_QUEUE_NAME } from '../queues/handover.queue.js';
...
async function loadHandoverSummary(accountId, handoverId) {
...
async function loadHandoverItems(accountId, handoverId) {
    const result = await db.$client.query(`
      select
        stay_id,
...
async function markBuilding(accountId, handoverId) {
...
async function markFailed(accountId, handoverId, buildError) {
    await db.$client.query(`
      update shift_handovers
      set
        build_status = 'failed',
...
async function writeStorageFile(storageDir, storageKey, content) {
...
export function createHandoverWorker(connection, prefix, options) {
    return new Worker(HANDOVER_BUILD_QUEUE_NAME, async (job) => {
...
//# sourceMappingURL=handover.worker.js.map