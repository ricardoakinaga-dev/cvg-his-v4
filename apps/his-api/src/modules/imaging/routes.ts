...
import {
  listImagingModalitiesQuerySchema, imagingModalityIdParamSchema, imagingModalityCreateSchema, imagingModalityUpdateSchema,
  listImagingTemplatesQuerySchema, imagingTemplateIdParamSchema, imagingTemplateCreateSchema, imagingTemplateUpdateSchema,
  listImagingOrdersQuerySchema, imagingOrderIdParamSchema, imagingOrderCreateSchema, imagingOrderUpdateSchema, imagingOrderScheduleSchema, imagingOrderCancelSchema,
  listImagingStudiesQuerySchema, imagingStudyIdParamSchema, imagingStudyCreateSchema, imagingStudyUpdateSchema, imagingStudyAttachDocumentSchema,
  listImagingReportsQuerySchema, imagingReportIdParamSchema, imagingReportCreateSchema, imagingReportUpdateSchema, imagingReportSignSchema, imagingReportAttachDocumentSchema,
  listImagingScheduleQuerySchema, imagingScheduleSlotCreateSchema, imagingScheduleSlotUpdateSchema
} from './types.js';
...
export const imagingRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/modalities/:id', { preHandler: requirePermission('imagem.modalidades.read') }, async (request, reply) => {
    ...
    if (!result) return reply.status(404).send({ message: 'Imaging modality not found' });
    return reply.send(result);
  });
  ...
  app.post('/modalities', { preHandler: requirePermission('imagem.modalidades.create') }, async (request, reply) => {
    ...
    if (result.kind === 'code_conflict') return reply.status(409).send({ message: 'Imaging modality code already exists' });
  ...
  app.put('/modalities/:id', { preHandler: requirePermission('imagem.modalidades.update') }, async (request, reply) => {
    ...
    if (result.kind === 'modality_not_found') return reply.status(404).send({ message: 'Imaging modality not found' });
    if (result.kind === 'code_conflict') return reply.status(409).send({ message: 'Imaging modality code already exists' });
  ...
  app.get('/templates/:id', { preHandler: requirePermission('imagem.templates.read') }, async (request, reply) => {
    ...
    const service = createImagingTemplatesService({ db: app.db, requestContext: request.requestContext });
    const result = await service.getById(params.id);
    if (!result) return reply.status(404).send({ message: 'Imaging template not found' });
    return reply.send(result);
  });
  ...
  app.put('/templates/:id', { preHandler: requirePermission('imagem.templates.update') }, async (request, reply) => {
    ...
    const service = createImagingTemplatesService({ db: app.db, requestContext: request.requestContext });
    const result = await service.update(params.id, body);
    if (result.kind === 'template_not_found') return reply.status(404).send({ message: 'Imaging template not found' });
  ...
  app.get('/orders/:id', { preHandler: requirePermission('imagem.pedidos.read') }, async (request, reply) => {
    ...
    const service = createImagingOrdersService({ db: app.db, requestContext: request.requestContext });
    const result = await service.getById(params.id);
    if (!result) return reply.status(404).send({ message: 'Imaging order not found' });
    return reply.send(result);
  });
  ...
  app.put('/orders/:id', { preHandler: requirePermission('imagem.pedidos.update') }, async (request, reply) => {
    ...
    const service = createImagingOrdersService({ db: app.db, requestContext: request.requestContext });
    const result = await service.update(params.id, body);
    if (result.kind === 'order_not_found') return reply.status(404).send({ message: 'Imaging order not found' });
  ...
  app.post('/orders/:id/schedule', { preHandler: requirePermission('imagem.pedidos.schedule') }, async (request, reply) => {
    ...
    if (result.kind === 'order_not_found') return reply.status(404).send({ message: 'Imaging order not found' });
    if (result.kind === 'invalid_status') return reply.status(400).send({ message: 'Cannot schedule order in current status' });
    return reply.send(result.order);
  });
  ...
  app.post('/orders/:id/start', { preHandler: requirePermission('imagem.estudos.create') }, async (request, reply) => {
    ...
    if (result.kind === 'order_not_found') return reply.status(404).send({ message: 'Imaging order not found' });
    if (result.kind === 'invalid_status') return reply.status(400).send({ message: 'Cannot start order in current status' });
    return reply.send(result.order);
  });
  ...
  app.post('/orders/:id/complete', { preHandler: requirePermission('imagem.estudos.update') }, async (request, reply) => {
    ...
    if (result.kind === 'order_not_found') return reply.status(404).send({ message: 'Imaging order not found' });
    if (result.kind === 'invalid_status') return reply.status(400).send({ message: 'Cannot complete order in current status' });
    return reply.send(result.order);
  });
  ...
  app.get('/studies/:id', { preHandler: requirePermission('imagem.estudos.read') }, async (request, reply) => {
    ...
    if (!result) return reply.status(404).send({ message: 'Imaging study not found' });
    return reply.send(result);
  });
  ...
  app.put('/studies/:id', { preHandler: requirePermission('imagem.estudos.update') }, async (request, reply) => {
    ...
    const service = createImagingStudiesService({ db: app.db, requestContext: request.requestContext });
    const result = await service.update(params.id, body);
    if (result.kind === 'study_not_found') return reply.status(404).send({ message: 'Imaging study not found' });
  ...
  app.get('/reports/:id', { preHandler: requirePermission('imagem.laudos.read') }, async (request, reply) => {
    ...
    const service = createImagingReportsService({ db: app.db, requestContext: request.requestContext });
    const result = await service.getByIdWithDocuments(params.id);
    if (!result) return reply.status(404).send({ message: 'Imaging report not found' });
    return reply.send(result);
  });
  ...
  app.put('/reports/:id', { preHandler: requirePermission('imagem.laudos.update') }, async (request, reply) => {
    ...
    const service = createImagingReportsService({ db: app.db, requestContext: request.requestContext });
    const result = await service.update(params.id, body);
    if (result.kind === 'report_not_found') return reply.status(404).send({ message: 'Imaging report not found' });
  ...
  app.put('/schedule/:id', { preHandler: requirePermission('imagem.agenda.manage') }, async (request, reply) => {
    ...
    const service = createImagingScheduleService({ db: app.db, requestContext: request.requestContext });
    const result = await service.update(params.id, body);
    if (result.kind === 'slot_not_found') return reply.status(404).send({ message: 'Schedule slot not found' });
  ...
  app.post('/schedule/:id/book', { preHandler: requirePermission('imagem.pedidos.schedule') }, async (request, reply) => {
    ...
    if (result.kind === 'slot_not_found') return reply.status(404).send({ message: 'Schedule slot not found' });
    if (result.kind === 'slot_not_available') return reply.status(400).send({ message: 'Schedule slot is not available' });
    return reply.send(result.slot);
  });
