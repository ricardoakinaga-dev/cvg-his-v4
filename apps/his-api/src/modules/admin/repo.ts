import { and, count, eq, ilike, inArray, desc, sql } from 'drizzle-orm';
...
export async function findUsers(
  db: typeof import('@cvg-his/db').db,
  accountId: string,
  options: {
    q?: string;
    page: number;
  ...
  if (q) {
    conditions.push(
      sql`(${users.email} ilike ${`%${q}%`} or ${users.fullName} ilike ${`%${q}%`})`
    );
  }
...
export async function findRoles(
  ...
  const permsByRole = new Map<string, Array<{ id: string; key: string }>>();
  ...
  const data: RoleResponse[] = rows.map((row) => ({
    id: row.id,
    name: row.name,
...
export async function findAuditEvents(
...
): Promise<{ data: AuditEventResponse[]; total: number }> {
  ...
  const data: AuditEventResponse[] = rows.map((row) => ({
    id: row.id,
    createdAt: row.createdAt.toISOString(),
