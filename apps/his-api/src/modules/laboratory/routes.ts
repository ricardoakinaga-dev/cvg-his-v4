...
import {
  listLabTestsQuerySchema, labTestIdParamSchema, labTestCreateSchema, labTestUpdateSchema,
  listLabOrdersQuerySchema, labOrderIdParamSchema, labOrderCreateSchema, labOrderUpdateSchema, labOrderCancelSchema,
  listLabSamplesQuerySchema, labSampleIdParamSchema, labSampleCreateSchema, labSampleCollectSchema, labSampleRejectSchema,
  listLabResultsQuerySchema, labResultIdParamSchema, labResultCreateSchema, labResultUpdateSchema,
  listLabReportsQuerySchema, labReportIdParamSchema, labReportCreateSchema, labReportUpdateSchema, labReportSignSchema,
  listLabReferenceRangesQuerySchema, labReferenceRangeIdParamSchema, labReferenceRangeCreateSchema, labReferenceRangeUpdateSchema
} from './types.js';

export const laboratoryRoutes: FastifyPluginAsync = async (app) => {
  // ============================================
  // LAB TESTS CATALOG
  // ============================================
  ...
  app.get('/tests/:id', { preHandler: requirePermission('laboratorio.catalogo.read') }, async (request, reply) => {
    ...
    if (!result) return reply.status(404).send({ message: 'Lab test not found' });
    return reply.send(result);
  });
  ...
  app.post('/tests', { preHandler: requirePermission('laboratorio.catalogo.create') }, async (request, reply) => {
    ...
    if (result.kind === 'code_conflict') return reply.status(409).send({ message: 'Lab test code already exists' });
  ...
  app.put('/tests/:id', { preHandler: requirePermission('laboratorio.catalogo.update') }, async (request, reply) => {
    ...
    if (result.kind === 'test_not_found') return reply.status(404).send({ message: 'Lab test not found' });
    if (result.kind === 'code_conflict') return reply.status(409).send({ message: 'Lab test code already exists' });
  ...
  app.get('/orders/:id', { preHandler: requirePermission('laboratorio.pedidos.read') }, async (request, reply) => {
    ...
    const service = createLabOrdersService({ db: app.db, requestContext: request.requestContext });
    const result = await service.getById(params.id);
    if (!result) return reply.status(404).send({ message: 'Lab order not found' });
    return reply.send(result);
  });
  ...
  app.put('/orders/:id', { preHandler: requirePermission('laboratorio.pedidos.update') }, async (request, reply) => {
    ...
    const service = createLabOrdersService({ db: app.db, requestContext: request.requestContext });
    const result = await service.update(params.id, body);
    if (result.kind === 'order_not_found') return reply.status(404).send({ message: 'Lab order not found' });
  ...
  app.post('/orders/:id/cancel', { preHandler: requirePermission('laboratorio.pedidos.cancel') }, async (request, reply) => {
    ...
    if (result.kind === 'order_not_found') return reply.status(404).send({ message: 'Lab order not found' });
    if (result.kind === 'invalid_status') return reply.status(400).send({ message: 'Cannot cancel order in current status' });
    return reply.send(result.order);
  });
  ...
  app.get('/samples/:id', { preHandler: requirePermission('laboratorio.coleta.read') }, async (request, reply) => {
    ...
    const service = createLabSamplesService({ db: app.db, requestContext: request.requestContext });
    const result = await service.getById(params.id);
    if (!result) return reply.status(404).send({ message: 'Lab sample not found' });
    return reply.send(result);
  });
  ...
  app.post('/samples', { preHandler: requirePermission('laboratorio.coleta.create') }, async (request, reply) => {
    ...
    if (!order) return reply.status(404).send({ message: 'Lab order not found' });
  ...
  app.post('/samples/:id/collect', { preHandler: requirePermission('laboratorio.coleta.update') }, async (request, reply) => {
    ...
    if (result.kind === 'sample_not_found') return reply.status(404).send({ message: 'Lab sample not found' });
    if (result.kind === 'invalid_status') return reply.status(400).send({ message: 'Sample cannot be collected in current status' });
  ...
  app.post('/samples/:id/receive', { preHandler: requirePermission('laboratorio.coleta.update') }, async (request, reply) => {
    ...
    if (result.kind === 'sample_not_found') return reply.status(404).send({ message: 'Lab sample not found' });
    if (result.kind === 'invalid_status') return reply.status(400).send({ message: 'Sample cannot be received in current status' });
    return reply.send(result.sample);
  });
  ...
  app.post('/samples/:id/reject', { preHandler: requirePermission('laboratorio.coleta.update') }, async (request, reply) => {
    ...
    if (result.kind === 'sample_not_found') return reply.status(404).send({ message: 'Lab sample not found' });
    if (result.kind === 'invalid_status') return reply.status(400).send({ message: 'Sample cannot be rejected in current status' });
    return reply.send(result.sample);
  });
  ...
  app.get('/results/:id', { preHandler: requirePermission('laboratorio.resultados.read') }, async (request, reply) => {
    ...
    const service = createLabResultsService({ db: app.db, requestContext: request.requestContext });
    const result = await service.getById(params.id);
    if (!result) return reply.status(404).send({ message: 'Lab result not found' });
    return reply.send(result);
  });
  ...
  app.put('/results/:id', { preHandler: requirePermission('laboratorio.resultados.update') }, async (request, reply) => {
    ...
    const service = createLabResultsService({ db: app.db, requestContext: request.requestContext });
    const result = await service.update(params.id, body);
    if (result.kind === 'result_not_found') return reply.status(404).send({ message: 'Lab result not found' });
  ...
  app.post('/results/:id/verify', { preHandler: requirePermission('laboratorio.resultados.verify') }, async (request, reply) => {
    ...
    if (result.kind === 'result_not_found') return reply.status(404).send({ message: 'Lab result not found' });
    if (result.kind === 'invalid_status') return reply.status(400).send({ message: 'Result cannot be verified in current status' });
    return reply.send(result.result);
  });
  ...
  app.get('/reports/:id', { preHandler: requirePermission('laboratorio.laudos.read') }, async (request, reply) => {
    ...
    const service = createLabReportsService({ db: app.db, requestContext: request.requestContext });
    const result = await service.getById(params.id);
    if (!result) return reply.status(404).send({ message: 'Lab report not found' });
    return reply.send(result);
  });
  ...
  app.put('/reports/:id', { preHandler: requirePermission('laboratorio.laudos.update') }, async (request, reply) => {
    ...
    const service = createLabReportsService({ db: app.db, requestContext: request.requestContext });
    const result = await service.update(params.id, body);
    if (result.kind === 'report_not_found') return reply.status(404).send({ message: 'Lab report not found' });
    if (result.kind === 'invalid_status') return reply.status(400).send({ message: 'Report cannot be updated in current status' });
  ...
  app.post('/reports/:id/finalize', { preHandler: requirePermission('laboratorio.laudos.update') }, async (request, reply) => {
    ...
    if (result.kind === 'report_not_found') return reply.status(404).send({ message: 'Lab report not found' });
    if (result.kind === 'invalid_status') return reply.status(400).send({ message: 'Report cannot be finalized in current status' });
    return reply.send(result.report);
  });
  ...
  app.post('/reports/:id/sign', { preHandler: requirePermission('laboratorio.laudos.sign') }, async (request, reply) => {
    ...
    if (result.kind === 'report_not_found') return reply.status(404).send({ message: 'Lab report not found' });
    if (result.kind === 'invalid_status') return reply.status(400).send({ message: 'Report cannot be signed in current status' });
    return reply.send(result.report);
  });
  ...
  app.get('/reference-ranges/:id', { preHandler: requirePermission('laboratorio.referencia.read') }, async (request, reply) => {
    ...
    const service = createLabReferenceRangesService({ db: app.db, requestContext: request.requestContext });
    const result = await service.getById(params.id);
    if (!result) return reply.status(404).send({ message: 'Reference range not found' });
    return reply.send(result);
  });
  ...
  app.put('/reference-ranges/:id', { preHandler: requirePermission('laboratorio.referencia.update') }, async (request, reply) => {
    ...
    const service = createLabReferenceRangesService({ db: app.db, requestContext: request.requestContext });
    const result = await service.update(params.id, body);
    if (result.kind === 'range_not_found') return reply.status(404).send({ message: 'Reference range not found' });
