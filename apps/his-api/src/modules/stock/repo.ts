...
export type StockLotsRepo = {
  ...
  }): Promise<{ items: StockLotWithProduct[]; total: number }>;
  ...
  create(input: { accountId: string; performedByUserId: string } & StockLotCreateInput): Promise<StockLotWithProduct>;
  updateById(params: {
    accountId: string;
    lotId: string;
  ...
  getProductBalance(accountId: string, productId: string): Promise<{ totalQuantity: string; totalActive: string }>;
};
...
export type StockMovementsRepo = {
  list(params: {
    ...
    startDate?: string;
    endDate?: string;
  }): Promise<{ items: StockMovementWithDetails[]; total: number }>;
  create(input: {
    accountId: string;
  ...
  getKardex(params: {
    ...
    page: number;
    pageSize: number;
  }): Promise<{ items: KardexEntry[]; total: number }>;
};
...
export function createStockLotsRepo(db: DbClient): StockLotsRepo {
  return {
    async list({ accountId, page, pageSize, productId, lotNumber, expiryWithinDays, includeExpired }) {
      ...
      if (expiryWithinDays && !includeExpired) {
        ...
        conditions.push(
          sql`${stockLots.expiryDate} IS NOT NULL AND ${stockLots.expiryDate} <= ${futureDate.toISOString().split('T')[0]}`
        );
      }
      ...
      if (!includeExpired) {
        ...
        conditions.push(
          sql`${stockLots.expiryDate} IS NULL OR ${stockLots.expiryDate} > ${today}`
        );
      }
      ...
      return { items: items as StockLotWithProduct[], total };
    },
    ...
    async findById(accountId, lotId) {
      ...
      return (lot as StockLotWithProduct) ?? null;
    },
    ...
    async findByProductAndLot(accountId, productId, lotNumber) {
      const [lot] = await db
        ...
        .where(
          and(
            eq(stockLots.accountId, accountId),
            eq(stockLots.productId, productId),
            eq(stockLots.lotNumber, lotNumber)
          )
        )
        .limit(1);
    ...
    async create(input) {
      ...
      return withProduct as StockLotWithProduct;
    },
    ...
    async updateById({ accountId, lotId, patch }) {
      ...
      if (patch.lotNumber !== undefined) updateData.lotNumber = patch.lotNumber;
      if (patch.expiryDate !== undefined) updateData.expiryDate = patch.expiryDate;
      if (patch.quantity !== undefined) updateData.quantity = patch.quantity.toString();
      if (patch.cost !== undefined) updateData.cost = patch.cost?.toString() ?? null;
      if (patch.location !== undefined) updateData.location = patch.location;
      if (patch.supplier !== undefined) updateData.supplier = patch.supplier;
      if (patch.notes !== undefined) updateData.notes = patch.notes;
      if (patch.active !== undefined) updateData.active = patch.active.toString();
    ...
    async getProductBalance(accountId, productId) {
      const [result] = await db
        .select({
          totalQuantity: sql<string>`COALESCE(SUM(${stockLots.quantity}), 0)`,
          totalActive: sql<string>`COALESCE(SUM(${stockLots.active}), 0)`
        })
        .from(stockLots)
        .where(
          and(
...
export function createStockMovementsRepo(db: DbClient): StockMovementsRepo {
  return {
    async list({
      accountId,
      page,
      ...
      if (movementType) {
        conditions.push(eq(stockMovements.movementType, movementType as 'entrada' | 'saida' | 'ajuste' | 'consumo' | 'devolucao' | 'transferencia'));
      }
      ...
      const items = await db
        .select({
          ...
          productSku: products.sku,
          lotNumber: stockLots.lotNumber,
          performedByName: sql<string>`${users.fullName}`
        })
        .from(stockMovements)
      ...
      return { items: items as StockMovementWithDetails[], total };
    },
    ...
    async create(input) {
      const [movement] = await db
        ...
        .values({
          ...
          movementType: input.movementType as 'entrada' | 'saida' | 'ajuste' | 'consumo' | 'devolucao' | 'transferencia',
      ...
      return withDetails as StockMovementWithDetails;
    },
    ...
    async getKardex({ accountId, productId, startDate, endDate, page, pageSize }) {
      ...
      return { items: items as KardexEntry[], total };
    }
  };
}
