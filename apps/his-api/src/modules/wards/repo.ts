...
function mapWardRow(row: Record<string, unknown>): WardRecord {
  return {
    id: String(row.id),
...
export function createWardsRepo(db: DbClient) {
  return {
    async create(input: CreateWardInput): Promise<WardRecord> {
      ...
      return mapWardRow(queryResult.rows[0] as Record<string, unknown>);
    },

    async findById(accountId: string, wardId: string): Promise<WardRecord | null> {
      const queryResult = await db.$client.query(
        'select * from wards where id = $1 and account_id = $2 limit 1',
        [wardId, accountId]
      );
    ...
    async updateById(
      accountId: string,
      wardId: string,
      patch: UpdateWardInput
    ): Promise<WardRecord | null> {
      const fields: string[] = [];
    ...
    async list(input: ListWardsInput) {
      ...
      return {
        data: rowsResult.rows.map((row) => mapWardRow(row as Record<string, unknown>)),
        page: input.page,
