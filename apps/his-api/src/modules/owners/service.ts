...
export function createOwnersService(context: ServiceContext) {
  const repo = createOwnersRepo(context.db);

  return {
    async create(input: CreateOwnerBody) {
    ...
    async update(ownerId: string, patch: UpdateOwnerBody) {
      const actor = ensureActor(context.requestContext);
    ...
    async list(query: ListOwnersQuery) {
      const actor = ensureActor(context.requestContext);
      return repo.list({
        accountId: actor.accountId,
        page: query.page,
