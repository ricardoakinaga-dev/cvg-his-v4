 ...
 */
function calculateAgeMonths(birthDate: string | null): number | null {
 ...
 */
function buildHighlightedAlerts(alerts: PatientAlerts): HighlightedAlerts {
  return {
 ...
 */
function mapPatientContextInfo(row: Record<string, unknown>): PatientContextInfo {
  const alertsJson = row.alerts_json;
  const alerts: PatientAlerts = typeof alertsJson === 'object' && alertsJson !== null 
    ? (alertsJson as PatientAlerts) 
    : {};
 ...
 */
function mapStayContextInfo(row: Record<string, unknown>): StayContextInfo {
  return {
    id: String(row.id),
    ...
    status: String(row.status) as 'active' | 'discharged' | 'transferred',
 ...
 */
function mapEncounterContextInfo(row: Record<string, unknown>): EncounterContextInfo {
  return {
    id: String(row.id),
    patientId: String(row.patient_id),
...
export function createPatientContextRepo(db: DbClient) {
  return {
    /**
     * Get patient context by patient ID
     */
    async getPatientContext(accountId: string, patientId: string): Promise<PatientContextInfo | null> {
      ...
      return mapPatientContextInfo(queryResult.rows[0] as Record<string, unknown>);
    },

    /**
     * Get active stay for a patient
     */
    async getActiveStayForPatient(accountId: string, patientId: string): Promise<StayContextInfo | null> {
      const queryResult = await db.$client.query(
        `
        SELECT 
          s.*,
      ...
      return mapStayContextInfo(queryResult.rows[0] as Record<string, unknown>);
    },

    /**
     * Get stay context by stay ID
     */
    async getStayContext(accountId: string, stayId: string): Promise<StayContextInfo | null> {
      const queryResult = await db.$client.query(
        `
        SELECT 
          s.*,
     ...
     */
    async getOpenEncounterForPatient(accountId: string, patientId: string): Promise<EncounterContextInfo | null> {
      const queryResult = await db.$client.query(
        `
        SELECT *
        FROM encounters
      ...
      return mapEncounterContextInfo(queryResult.rows[0] as Record<string, unknown>);
    },

    /**
     * Get patient context by stay ID (includes patient info)
     */
    async getPatientContextByStay(accountId: string, stayId: string): Promise<{
      ...
      const stay: StayContextInfo = {
        ...
        bedId: String(row.bed_id),
        bedName: String(row.bed_name),
        status: String(row.stay_status) as 'active' | 'discharged' | 'transferred',
        admittedAt: String(row.admitted_at),
        dischargedAt: row.discharged_at ? String(row.discharged_at) : null,
        chiefComplaint: row.chief_complaint ? String(row.chief_complaint) : null,
     ...
     */
    async getNavigationCounts(accountId: string, patientId: string, stayId?: string): Promise<{
      activeOrders: number;
      ...
      if (stayId) {
        ...
        pendingAdminCount = Number((adminResult.rows[0] as Record<string, unknown>)?.count ?? 0);
      }
      ...
      return {
        activeOrders: Number((ordersResult.rows[0] as Record<string, unknown>)?.count ?? 0),
        pendingAdministrations: pendingAdminCount,
        unsignedNotes: Number((notesResult.rows[0] as Record<string, unknown>)?.count ?? 0),
      };
    },
  };
}
