...
function mapOrderStatus(value: unknown): MedicationLogOrderRow['status'] {
  return String(value) === 'stopped' ? 'stopped' : 'active';
}

function mapAdministrationStatus(value: unknown): MedicationLogAdministrationRow['status'] {
...
function mapOrderRow(row: Record<string, unknown>): MedicationLogOrderRow {
  return {
    id: String(row.id),
...
function mapAdministrationRow(row: Record<string, unknown>): MedicationLogAdministrationRow {
...
export type MedicationLogsRepo = {
  ...
  listRecentAdministrationsByStay: (
    accountId: string,
    stayId: string,
    limit: number
  ) => Promise<MedicationLogAdministrationRow[]>;
};

export function createMedicationLogsRepo(db: DbClient): MedicationLogsRepo {
  return {
    async listActiveOrdersByStay(accountId: string, stayId: string): Promise<MedicationLogOrderRow[]> {
      ...
      return queryResult.rows.map((row) => mapOrderRow(row as Record<string, unknown>));
    },

    async listRecentAdministrationsByStay(
      accountId: string,
      stayId: string,
      limit: number
    ): Promise<MedicationLogAdministrationRow[]> {
      ...
      return queryResult.rows.map((row) => mapAdministrationRow(row as Record<string, unknown>));
    }
  };
}
