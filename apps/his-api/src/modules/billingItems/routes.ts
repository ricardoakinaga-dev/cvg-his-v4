...
import {
  ...
  billingItemIdParamSchema,
  listBillingItemsQuerySchema,
  billingItemCreateSchema,
  billingItemUpdateSchema
} from './types.js';

export const billingItemsRoutes: FastifyPluginAsync = async (app) => {
  // List billing items for an encounter
  app.get(
    '/:encounterId/billing-items',
    {
  ...
  app.post(
    '/:encounterId/billing-items/confirm-all',
    {
      preHandler: requirePermission('financeiro.comandas.update')
    },
    async (request, reply) => {
