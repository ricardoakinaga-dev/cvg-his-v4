...
function mapMedicationOrderRow(row) {
    return {
        id: String(row.id),
        ...
        prescriptionText: row.prescription_text === null || row.prescription_text === undefined
            ? null
...
export function createMedicationOrdersRepo(db) {
    return {
        ...
        async findStayInAccount(accountId, stayId) {
            const queryResult = await db.$client.query(`
        ...
        `, [
            ...
            return mapMedicationOrderRow(queryResult.rows[0]);
        },
        ...
        async list(input) {
          ...
          `, values)
            ...
            return {
                data: rowsResult.rows.map((row) => mapMedicationOrderRow(row)),
                page: input.page,
