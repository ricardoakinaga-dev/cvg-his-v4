...
describe('protocol publish service', () => {
    ...
    it('compensates to failed when enqueue fails after publishing state transition', async () => {
        ...
        const service = createProtocolPublishService({ db: fakeDb, requestContext: createRequestContext() }, { repo, appendAudit, enqueueProtocolPublish });
        await expect(service.requestPublish(before.id)).rejects.toMatchObject({
            code: 'QUEUE_UNAVAILABLE',
            statusCode: 503
        });
        expect(repo.markFailed).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('retries publish successfully from failed state and re-queues job', async () => {
        ...
        expect(repo.markPublishing).toHaveBeenCalledWith(expect.objectContaining({
            accountId: before.accountId,
            versionId: before.id
        }));
        expect(enqueueProtocolPublish).toHaveBeenCalledWith(expect.objectContaining({
            accountId: before.accountId,
            versionId: before.id,
