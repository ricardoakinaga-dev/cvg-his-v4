function mapOwnerRow(row) {
    ...
    return {
        ...
        addressJson: typeof addressJsonRaw === 'object' && addressJsonRaw !== null
            ? addressJsonRaw
            : null,
...
export function createOwnersRepo(db) {
    return {
        async create(input) {
        ...
        `, [
            ...
            return mapOwnerRow(queryResult.rows[0]);
        },
        async findById(accountId, ownerId) {
            const queryResult = await db.$client.query('select * from owners where id = $1 and account_id = $2 limit 1', [ownerId, accountId]);
            if (queryResult.rows.length === 0) {
                return null;
            }
            return mapOwnerRow(queryResult.rows[0]);
        },
        async updateById(accountId, ownerId, patch) {
            const fields = [];
            const values = [];
            let index = 1;
        ...
        async list(input) {
            ...
            let index = 2;
            if (input.q) {
                whereParts.push(`(full_name ilike $${index} or document ilike $${index} or phone_main ilike $${index} or phone_alt ilike $${index})`);
                values.push(`%${input.q}%`);
                index += 1;
            }
          ...
          `, [...values, input.pageSize, offset]),
            ...
            return {
                data: rowsResult.rows.map((row) => mapOwnerRow(row)),
                page: input.page,
