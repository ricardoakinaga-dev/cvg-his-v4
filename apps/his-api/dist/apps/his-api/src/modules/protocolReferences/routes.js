...
export const protocolReferencesRoutes = async (app) => {
    const suggestAdapter = app.env.QDRANT_URL
        ...
        : null;
    app.get('/:id/references', {
        preHandler: requirePermission('protocol.ref.read')
    }, async (request, reply) => {
        const params = protocolIdParamSchema.parse(request.params);
        const service = createProtocolReferencesService({ db: app.db, requestContext: request.requestContext }, { suggestAdapter });
    ...
    app.delete('/:id/references/:refId', {
        preHandler: requirePermission('protocol.ref.write')
    }, async (request, reply) => {
