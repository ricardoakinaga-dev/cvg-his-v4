...
import { ownerContacts, ownerAddresses, ownerDocuments, ownerAlerts, patientAlerts, patientVaccines, patientAllergies, tags, ownerTags, patientTags, owners, patients } from '@cvg-his/db';
// ============================================
// OWNER CONTACTS REPO
// ============================================
export function createOwnerContactsRepo(db) {
    return {
        async create(accountId, ownerId, data) {
        ...
        async findByOwner(accountId, ownerId) {
            return db.select()
                .from(ownerContacts)
                .where(and(eq(ownerContacts.accountId, accountId), eq(ownerContacts.ownerId, ownerId)))
                .orderBy(desc(ownerContacts.isPrimary));
        },
        async findById(accountId, id) {
            const [result] = await db.select()
                .from(ownerContacts)
                .where(and(eq(ownerContacts.accountId, accountId), eq(ownerContacts.id, id)));
            return result ?? null;
        },
...
// ============================================
export function createOwnerAddressesRepo(db) {
    return {
        async create(accountId, ownerId, data) {
        ...
        async findByOwner(accountId, ownerId) {
            return db.select()
                .from(ownerAddresses)
                .where(and(eq(ownerAddresses.accountId, accountId), eq(ownerAddresses.ownerId, ownerId)))
                .orderBy(desc(ownerAddresses.isPrimary));
        },
        async findById(accountId, id) {
            const [result] = await db.select()
                .from(ownerAddresses)
                .where(and(eq(ownerAddresses.accountId, accountId), eq(ownerAddresses.id, id)));
            return result ?? null;
        },
        async update(accountId, id, data) {
...
// ============================================
export function createOwnerDocumentsRepo(db) {
    return {
        async create(accountId, ownerId, data) {
        ...
        async findByOwner(accountId, ownerId) {
            return db.select()
                .from(ownerDocuments)
                .where(and(eq(ownerDocuments.accountId, accountId), eq(ownerDocuments.ownerId, ownerId)));
        },
        async findById(accountId, id) {
            const [result] = await db.select()
                .from(ownerDocuments)
                .where(and(eq(ownerDocuments.accountId, accountId), eq(ownerDocuments.id, id)));
            return result ?? null;
        },
        async update(accountId, id, data) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
// OWNER ALERTS REPO
// ============================================
export function createOwnerAlertsRepo(db) {
    return {
        async create(accountId, ownerId, data) {
        ...
        async findByOwner(accountId, ownerId, activeOnly = false) {
            const conditions = [
        ...
        async findById(accountId, id) {
            const [result] = await db.select()
                .from(ownerAlerts)
                .where(and(eq(ownerAlerts.accountId, accountId), eq(ownerAlerts.id, id)));
            return result ?? null;
        },
        async update(accountId, id, data) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            if (data.resolvedByUserId !== undefined)
                updateData.resolvedByUserId = data.resolvedByUserId;
        ...
        async resolve(accountId, id, resolvedByUserId) {
            const [updated] = await db.update(ownerAlerts)
                .set({
                isActive: false,
                resolvedAt: new Date(),
...
// PATIENT ALERTS REPO
// ============================================
export function createPatientAlertsRepo(db) {
    return {
        async create(accountId, patientId, data) {
        ...
        async findByPatient(accountId, patientId, activeOnly = false) {
            const conditions = [
        ...
        async findById(accountId, id) {
            const [result] = await db.select()
                .from(patientAlerts)
                .where(and(eq(patientAlerts.accountId, accountId), eq(patientAlerts.id, id)));
            return result ?? null;
        },
        async update(accountId, id, data) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
// ============================================
export function createPatientVaccinesRepo(db) {
    return {
        async create(accountId, patientId, data) {
        ...
        async findByPatient(accountId, patientId) {
            return db.select()
                .from(patientVaccines)
                .where(and(eq(patientVaccines.accountId, accountId), eq(patientVaccines.patientId, patientId)))
                .orderBy(desc(patientVaccines.administrationDate));
        },
        async findById(accountId, id) {
            const [result] = await db.select()
                .from(patientVaccines)
                .where(and(eq(patientVaccines.accountId, accountId), eq(patientVaccines.id, id)));
            return result ?? null;
        },
        async update(accountId, id, data) {
        ...
        async findUpcoming(accountId, days = 30) {
            const futureDate = new Date();
            futureDate.setDate(futureDate.getDate() + days);
            return db.select()
                .from(patientVaccines)
                .where(and(eq(patientVaccines.accountId, accountId), sql `${patientVaccines.nextDoseDate} <= ${futureDate}`, sql `${patientVaccines.nextDoseDate} >= ${new Date()}`))
...
// ============================================
export function createPatientAllergiesRepo(db) {
    return {
        async create(accountId, patientId, data) {
        ...
        async findById(accountId, id) {
            const [result] = await db.select()
                .from(patientAllergies)
                .where(and(eq(patientAllergies.accountId, accountId), eq(patientAllergies.id, id)));
            return result ?? null;
        },
        async update(accountId, id, data) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            if (data.diagnosedDate !== undefined)
                updateData.diagnosedDate = data.diagnosedDate;
            if (data.notes !== undefined)
                updateData.notes = data.notes;
            if (data.isActive !== undefined)
                updateData.isActive = data.isActive;
...
// TAGS REPO
// ============================================
export function createTagsRepo(db) {
    return {
        ...
        async addToOwner(ownerId, tagId) {
            const [created] = await db.insert(ownerTags)
                .values({ ownerId, tagId })
                .returning();
            return created;
        },
        async removeFromOwner(ownerId, tagId) {
            const [deleted] = await db.delete(ownerTags)
                .where(and(eq(ownerTags.ownerId, ownerId), eq(ownerTags.tagId, tagId)))
                .returning();
            return deleted ?? null;
        },
        async addToPatient(patientId, tagId) {
            const [created] = await db.insert(patientTags)
                .values({ patientId, tagId })
                .returning();
            return created;
        },
        async removeFromPatient(patientId, tagId) {
            const [deleted] = await db.delete(patientTags)
                .where(and(eq(patientTags.patientId, patientId), eq(patientTags.tagId, tagId)))
                .returning();
            return deleted ?? null;
        },
        async findOwnerTags(accountId, ownerId) {
            return db.select({ tag: tags })
                .from(ownerTags)
                .innerJoin(tags, eq(ownerTags.tagId, tags.id))
                .where(and(eq(tags.accountId, accountId), eq(ownerTags.ownerId, ownerId)));
        },
        async findPatientTags(accountId, patientId) {
            return db.select({ tag: tags })
                .from(patientTags)
                .innerJoin(tags, eq(patientTags.tagId, tags.id))
                .where(and(eq(tags.accountId, accountId), eq(patientTags.patientId, patientId)));
        }
    };
}
// ============================================
// SEARCH REPO
// ============================================
export function createSearchRepo(db) {
    return {
        async search(accountId, query, limit) {
            ...
            const [ownerResults, patientResults] = await Promise.all([
                db.select({
                    ...
                    .from(owners)
                    .where(and(eq(owners.accountId, accountId), or(ilike(owners.fullName, searchPattern), ilike(owners.document, searchPattern), ilike(owners.phoneMain, searchPattern), ilike(owners.email, searchPattern))))
                    .limit(limit),
                db.select({
                    id: patients.id,
                    name: patients.name,
                    ...
                    .where(and(eq(patients.accountId, accountId), or(ilike(patients.name, searchPattern), ilike(patients.microchip, searchPattern), ilike(patients.registrationNumber, searchPattern))))
                    .limit(limit)
            ]);
            return {
                owners: ownerResults.map(o => ({ ...o, type: 'owner' })),
                patients: patientResults.map(p => ({ ...p, type: 'patient' }))
            };
        }
    };
}
