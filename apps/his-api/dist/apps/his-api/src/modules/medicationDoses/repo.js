...
function mapDueOrderScheduleRow(row) {
    return {
...
export function createMedicationDosesRepo(db) {
    return {
        async listActiveOrderSchedules(accountId, stayId) {
        ...
        `, values);
            return queryResult.rows.map((row) => mapDueOrderScheduleRow(row));
        },
        async listLastScheduledForByOrderIds(accountId, orderIds) {
        ...
        async listAdministrationRowsInWindow(accountId, orderIds, from, to) {
            if (orderIds.length === 0) {
                return [];
            }
            const queryResult = await db.$client.query(`
        ...
        async updateScheduleNextDueAt(accountId, scheduleId, nextDueAt) {
