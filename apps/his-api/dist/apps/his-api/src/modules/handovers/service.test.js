...
const fakeDb = {};
function makeHandoverRecord(overrides = {}) {
    return {
...
function makeHandoverWithItems(overrides = {}) {
    return {
        handover: makeHandoverRecord(overrides),
        items: []
    };
}
function makeValidHandoverItem(overrides = {}) {
    return {
...
describe('handovers service', () => {
    ...
    it('compensates build_status to failed when enqueue fails after publish transition', async () => {
        ...
        const service = createHandoversService({ db: fakeDb, requestContext: createRequestContext() }, { repo, appendAudit, enqueueHandoverBuild });
        await expect(service.publish(before.handover.id)).rejects.toMatchObject({
            code: 'QUEUE_UNAVAILABLE',
            statusCode: 503
        });
        expect(repo.markBuildFailed).toHaveBeenCalledWith(expect.objectContaining({
