<template>
  <div class="laboratory-hemogram-reference-values-page">
    <AppPageHeader
      :breadcrumbs="['Laboratório', 'Cadastros', 'Vlr. Ref. Hemograma']"
      title="Vlr. Ref. Hemograma"
      subtitle="Cadastro dos intervalos hematológicos usados na interpretação de hemogramas"
    >
      <template #actions>
        <DsButton variant="primary" tag="a" to="/laboratory/hemogram-reference-values/new" icon="➕">Incluir</DsButton>
    ...
    <section class="hub-kpis" aria-label="Resumo dos valores de referência de hemograma">
      ...
      <DsStatCard label="HEM" value="" icon="🩸" />
    </section>

    <section class="filter-panel" aria-label="Filtros de valores de referência de hemograma">
    ...
    >
      ...
      <template #cell-actions="{ row }">
        <DsButton
          tag="a"
          :to="`/laboratory/hemogram-reference-values/${(row as ReferenceValueRow).id}`"
          size="sm"
          variant="secondary"
        >
          Abrir
...
async function load() {
  ...
  error.value = '';
  try {
    referenceValues.value = await laboratoryService.listHemogramReferenceValues({
      id: appliedFilters.id || undefined,
      parameter: appliedFilters.parameter || undefined,
      unit: appliedFilters.unit || undefined
    });
  } catch (err: unknown) {
    error.value = err instanceof Error ? err.message : 'Erro ao carregar valores de referência do hemograma';
...
<style scoped>
.laboratory-hemogram-reference-values-page {
  display: flex;
