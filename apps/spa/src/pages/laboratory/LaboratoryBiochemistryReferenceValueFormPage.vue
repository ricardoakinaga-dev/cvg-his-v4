<template>
  <div class="laboratory-biochemistry-reference-value-form-page">
    <AppPageHeader
      :breadcrumbs="['Laboratório', 'Cadastros', 'Vlr. Ref. Bioquímico', isEditing ? 'Editar' : 'Incluir']"
      :title="isEditing ? 'Editar Valor Bioquímico' : 'Incluir Valor Bioquímico'"
      subtitle="Parâmetros bioquímicos usados para destacar resultados abaixo, dentro ou acima da faixa"
    >
      <template #actions>
        <DsButton variant="secondary" tag="a" to="/laboratory/biochemistry-reference-values">Voltar</DsButton>
      </template>
    </AppPageHeader>
      ...
      <DsCard>
        <form class="reference-form" @submit.prevent="submitForm">
          <label class="form-field">
            <span>Parâmetro</span>
            <input v-model="form.parameter" required autocomplete="off" placeholder="Ex: ALT" />
          </label>
          <label class="form-field">
            <span>Unidade</span>
            <input v-model="form.unit" required autocomplete="off" placeholder="Ex: U/L" />
          </label>
          <label class="form-field">
      ...
      <aside class="form-aside">
        ...
        <DsCard title="Integrações operacionais">
          <div class="detail-list">
            <div><strong>Bioquímico:</strong> faixa usada no resultado estruturado por painel.</div>
            <div><strong>Laudos:</strong> apoio para interpretação clínica do perfil sérico.</div>
...
async function submitForm() {
  ...
  try {
    ...
    if (isEditing.value && referenceValueId.value) {
      ...
      await laboratoryService.createBiochemistryReferenceValue(payload);
    }
...
<style scoped>
.laboratory-biochemistry-reference-value-form-page {
  display: flex;
