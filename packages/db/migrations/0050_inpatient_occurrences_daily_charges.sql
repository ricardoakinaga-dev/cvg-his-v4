...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'inpatient_occurrences_type_chk'
  ) THEN
    ALTER TABLE inpatient_occurrences
      ADD CONSTRAINT inpatient_occurrences_type_chk
      CHECK (type IN ('clinical', 'nursing', 'medication', 'feeding', 'behavior', 'administrative'));
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM information_schema.table_constraints
    WHERE constraint_name = 'inpatient_occurrences_severity_chk'
  ) THEN
    ALTER TABLE inpatient_occurrences
      ADD CONSTRAINT inpatient_occurrences_severity_chk
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM information_schema.table_constraints
    WHERE constraint_name = 'inpatient_daily_charges_status_chk'
  ) THEN
    ALTER TABLE inpatient_daily_charges
      ADD CONSTRAINT inpatient_daily_charges_status_chk
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM information_schema.table_constraints
    WHERE constraint_name = 'inpatient_daily_charges_amount_chk'
  ) THEN
    ALTER TABLE inpatient_daily_charges
      ADD CONSTRAINT inpatient_daily_charges_amount_chk
      CHECK (quantity > 0 AND unit_amount >= 0 AND total_amount >= 0);
  END IF;
END $$;
