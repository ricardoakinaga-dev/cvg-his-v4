...
const rolePermissionMap: Record<string, string[]> = Object.fromEntries(
  Object.entries(ROLE_PERMISSIONS).map(([roleName, rolePerms]) => [
    roleName,
    [...rolePerms]
  ])
);
...
async function seedSettings(accountId: string): Promise<void> {
...
async function seedProducts(accountId: string): Promise<void> {
...
async function seedServices(accountId: string): Promise<void> {
...
async function seedAgenda(accountId: string, unitId: string): Promise<void> {
  const collabs = [
