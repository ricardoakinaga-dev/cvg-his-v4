...
export const encounterFinancialStatusEnum = pgEnum('encounter_financial_status', [
  'pending',
  'partial',
  'paid'
]);
...
export const encounterFinancialAccounts = pgTable(
  'encounter_financial_accounts',
  {
    ...
    subtotalSnapshot: numeric('subtotal_snapshot', { precision: 12, scale: 2 })
      .notNull()
      .default('0'),
    discountTotalSnapshot: numeric('discount_total_snapshot', { precision: 12, scale: 2 })
      .notNull()
      .default('0'),
  ...
  (table) => ({
    ...
    accountIdIdUnique: uniqueIndex('idx_efa_account_id_id_unique').on(table.accountId, table.id),
    accountStatusIdx: index('idx_efa_account_status').on(table.accountId, table.financialStatus)
  })
);

export const encounterReceivableStatusEnum = pgEnum('encounter_receivable_status', [
  'open',
  'settled'
]);
...
export const encounterReceivables = pgTable(
  ...
  (table) => ({
    accountIdIdUnique: uniqueIndex('idx_er_account_id_id_unique').on(table.accountId, table.id),
    financialInstallmentUniqueIdx: uniqueIndex('uidx_er_financial_installment').on(
      table.financialAccountId,
      table.installmentNumber
    ),
    accountStatusIdx: index('idx_er_account_status').on(table.accountId, table.status),
    accountEncounterStatusIdx: index('idx_er_account_encounter_status').on(
      table.accountId,
      table.encounterId,
      table.status
    ),
...
export const encounterReceivablePayments = pgTable(
  'encounter_receivable_payments',
  {
    ...
    externalReferenceType: varchar('external_reference_type', { length: 64 }),
    externalReferenceId: varchar('external_reference_id', { length: 255 }),
    notes: text('notes'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow()
  },
  (table) => ({
    accountIdIdUnique: uniqueIndex('idx_erp_account_id_id_unique').on(table.accountId, table.id),
    accountReceivablePaidAtIdx: index('idx_erp_account_receivable_paid_at').on(
      table.accountId,
      table.receivableId,
      table.paidAt
    ),
    accountExternalReferenceIdx: index('idx_erp_account_external_reference').on(
      table.accountId,
