...
async function getDrizzleMigrationCount(client: PoolClient): Promise<number> {
  try {
    const result = await client.query(
...
export async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    if (!hasAccountsBefore && migrationCount > 0 && process.env.NODE_ENV !== 'production') {
      console.warn(
        `[db:migrate] Detected empty public schema with stale drizzle migration history (${migrationCount} rows). Resetting drizzle.__drizzle_migrations in non-production mode.`
      );
    ...
    if (!hasAccountsAfter) {
      throw new Error(
        'Migrations finished but public.accounts is missing. Check drizzle.__drizzle_migrations state and rerun migrations.'
      );
    }
