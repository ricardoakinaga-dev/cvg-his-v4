 ...
 */
export function calculateDoseStatus(input: DoseDueInput): DoseDueResult {
  const {
    now,
    scheduledFor,
    delayedUntil,
 ...
 */
export function calculateDelaySeverity(delayMinutes: number): 'low' | 'medium' | 'high' {
  if (delayMinutes >= 120) {
    return 'high';
  }
 ...
 */
export function shouldTriggerOverdueAlert(
  status: DoseStatus,
  delayMinutes: number,
...
export function categorizeDoses<T extends { scheduledFor: Date; delayedUntil?: Date | null; administrationStatus?: AdministrationStatus | null }>(
  doses: T[],
  now: Date,
  graceMinutes: number = DEFAULT_GRACE_MINUTES
): CategorizedDoses<T> {
  const result: CategorizedDoses<T> = {
    overdue: [],
    due: [],
    upcoming: [],
    administered: [],
