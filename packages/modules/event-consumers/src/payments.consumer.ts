...
export class PaymentsEventHandlers {
  ...
  async #handleCardCompleted(event: OutboxEvent): Promise<void> {
    ...
    // billing record: an unknown/replayed provider event must fail and retry.
    if (!transaction) {
      throw new Error(
        `Card completion ${payload.intentId} has no authoritative transaction intent`
      );
    }
    ...
    if (billingRecord) {
      if (
        transaction.billingRecordId !== effectiveBillingRecordId ||
        billingRecord.accountId !== payload.accountId
      ) {
        throw new Error('Card completion does not match the billing record account');
      }
      if (
        transaction.currency !== billingRecord.currency ||
        transaction.amount !== billingRecord.subtotalAmount
      ) {
        throw new Error('Card completion amount does not match the billing record');
      }
    }
