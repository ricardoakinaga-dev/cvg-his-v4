...
test('NotificationsService: processPending moves notifications to sent', async () => {
  ...
  const queued = notifications.list('acc_1' as never, 'queued');
  assert.equal(queued.length, 2);

  const processed = await notifications.processPending('acc_1' as never, { limit: 10 });
  assert.equal(processed.length, 2);

  const sent = notifications.list('acc_1' as never, 'sent');
  assert.equal(sent.length, 2);

  const stillQueued = notifications.list('acc_1' as never, 'queued');
...
test('NotificationsService: processPending respects limit', async () => {
  ...
  const processed = await notifications.processPending('acc_1' as never, { limit: 2 });
  assert.equal(processed.length, 2);

  const remaining = notifications.list('acc_1' as never, 'queued');
...
test('NotificationsService: list filters by status', async () => {
  ...
  await notifications.processPending('acc_1' as never, { limit: 1 });
...
test('NotificationsService: listJobs returns job entries', async () => {
  ...
  const jobs = notifications.listJobs('acc_1' as never);
...
test('NotificationsService: repository-backed list and processing stay account-scoped', async () => {
  ...
  const repository = {
    ...
    async findQueuedJobs(accountId: any, limit: any) {
  ...
  const scopedList = await notifications.listFromRepository('acc_1' as never, 'queued');
  assert.equal(scopedList.length, 1);
  assert.equal(scopedList[0].id, first.id);

  const processed = await notifications.processPendingFromRepository('acc_1' as never, {
    limit: 10
  });
  assert.equal(processed.length, 1);
  assert.equal(processed[0].id, first.id);

  const acc1Jobs = await notifications.listJobsFromRepository('acc_1' as never);
  const acc2Jobs = await notifications.listJobsFromRepository('acc_2' as never);
...
test('NotificationsService: processPendingFromRepository fires onNotificationSent callback', async () => {
  ...
  const repository = {
    ...
    async findQueuedJobs(_accountId: any, limit: any) {
      return Array.from(jobsStore.values())
