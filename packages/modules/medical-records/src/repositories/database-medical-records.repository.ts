...
function isUninitializedTestDouble(): boolean {
  if (process.env.NODE_ENV !== 'test' || getTenantContext()) return false;
  try {
    getPool();
    return false;
  } catch (error) {
    return (
      ...
      error.message === 'Database pool not initialized. Call createDatabaseClient first.'
    );
  }
}

function resolveAccountId(explicitAccountId?: string): string | undefined {
  const contextAccountId = getTenantContext()?.accountId;
  if (contextAccountId) {
    if (explicitAccountId && explicitAccountId !== contextAccountId) {
      throw new Error('Medical-record account does not match tenant context');
    }
    return contextAccountId;
  }
  if (isUninitializedTestDouble()) return explicitAccountId;
...
async function withTenantDatabase<T>(
  ...
  operation: (scopedDatabase: DatabaseClient) => Promise<T>
): Promise<T> {
  if (isUninitializedTestDouble()) return operation(database);
  return withTenantTransaction(accountId ?? requireAccountId(), operation);
}
...
export class DatabaseMedicalRecordRepository implements MedicalRecordRepository {
  ...
  public async create(record: MedicalRecordSummary): Promise<void> {
    ...
    await withTenantDatabase(this.#db, accountId, (database) =>
      database.insert(medicalRecords).values({
        id: record.id,
  ...
  public async findAll(accountId: AccountId): Promise<readonly MedicalRecordSummary[]> {
    let result: (typeof medicalRecords.$inferSelect)[];
    try {
      result = await withTenantDatabase(this.#db, resolveAccountId(accountId), (database) =>
        database.select().from(medicalRecords).where(eq(medicalRecords.accountId, accountId))
      );
    } catch (error) {
      if (allowSchemaCompatibilityFallback() && isMissingRelationError(error)) {
        return [];
...
export class DatabaseClinicalEntryRepository implements ClinicalEntryRepository {
  ...
  public async create(entry: ClinicalEntrySummary): Promise<void> {
    ...
    await withTenantDatabase(this.#db, accountId, (database) =>
      database.insert(clinicalEntries).values({
        id: entry.id,
  ...
  public async findById(entryId: ClinicalEntryId): Promise<ClinicalEntrySummary | null> {
    ...
    try {
      result = await withTenantDatabase(this.#db, resolveAccountId(), (database) =>
        database.select().from(clinicalEntries).where(eq(clinicalEntries.id, entryId)).limit(1)
      );
    } catch (error) {
      if (allowSchemaCompatibilityFallback() && isMissingRelationError(error)) {
        return null;
...
export class DatabaseClinicalTimelineRepository implements ClinicalTimelineRepository {
  ...
  public async create(event: ClinicalTimelineEventSummary): Promise<void> {
    const accountId = resolveAccountId(event.accountId);
    await withTenantDatabase(this.#db, accountId, (database) =>
      database.insert(clinicalTimeline).values({
        id: event.id,
...
export class DatabaseEntryRevisionRepository implements EntryRevisionRepository {
  ...
  public async create(revision: EntryRevisionSummary): Promise<void> {
    await withTenantDatabase(this.#db, resolveAccountId(), (database) =>
      database.insert(entryRevisions).values({
