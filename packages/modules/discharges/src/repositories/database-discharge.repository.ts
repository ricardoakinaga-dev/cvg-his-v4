...
export class DatabaseDischargeRepository implements DischargeRepository {
  ...
  async findById(id: DischargeId): Promise<DischargeSummary | null> {
    const pool = getPool();
    const result = await pool.query('SELECT * FROM discharges WHERE id = $1', [id]);
  ...
  async findByEncounterId(encounterId: EncounterId): Promise<DischargeSummary | null> {
    const pool = getPool();
    const result = await pool.query('SELECT * FROM discharges WHERE encounter_id = $1', [encounterId]);
    if (result.rows.length === 0) return null;
    return this.mapRow(result.rows[0]);
  }
  ...
  async delete(id: DischargeId): Promise<void> {
    const pool = getPool();
    await pool.query('DELETE FROM discharges WHERE id = $1', [id]);
  }
