...
export class DatabaseAccessControlRepository implements AccessControlRepository {
  ...
  async updateTeam(
    ...
    if (!existing) throw new Error(`Access team not found: ${id}`);
    const pool = getPool();
    const now = nowIso();
    await pool.query(
      `UPDATE access_teams
  ...
  async updateSector(
    ...
    const existing = await this.findSectorById(id);
    if (!existing) throw new Error(`Access sector not found: ${id}`);
    const pool = getPool();
    const now = nowIso();
    await pool.query(
  ...
  async replaceUserTeams(userId: UserId, teamIds: readonly AccessTeamId[]): Promise<void> {
    const pool = getPool();
    await pool.query('DELETE FROM access_team_memberships WHERE user_id = $1', [userId]);
  ...
  async replaceUserSectors(userId: UserId, sectorIds: readonly AccessSectorId[]): Promise<void> {
    const pool = getPool();
    await pool.query('DELETE FROM access_sector_memberships WHERE user_id = $1', [userId]);
  ...
  async upsertPermissionAssignment(input: {
    ...
    const pool = getPool();
    const permission = await this.findPermissionByKey(input.permissionCode);
    if (!permission) throw new Error(`Permission not found: ${input.permissionCode}`);
    const now = nowIso();
  ...
  private async findTeamById(id: AccessTeamId): Promise<AccessTeamSummary | null> {
    const pool = getPool();
    const result = await pool.query(`SELECT * FROM access_teams WHERE id = $1`, [id]);
    if (result.rows.length === 0) return null;
    return this.mapTeam(result.rows[0]);
  }

  private async findSectorById(id: AccessSectorId): Promise<AccessSectorSummary | null> {
    const pool = getPool();
    const result = await pool.query(`SELECT * FROM access_sectors WHERE id = $1`, [id]);
    if (result.rows.length === 0) return null;
    return this.mapSector(result.rows[0]);
  }
