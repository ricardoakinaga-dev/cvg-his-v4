...
beforeAll(async () => {
  ...
  for (const role of roles) {
    ...
    await pool.query(`GRANT USAGE ON SCHEMA public, app TO "${role}"`);
    await pool.query(`GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON ALL TABLES IN SCHEMA public TO "${role}"`);
  }
});
