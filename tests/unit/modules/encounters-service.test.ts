...
describe('EncountersService coverage guard', () => {
  it('rejects duplicate active encounters for the same patient and validates transitions/closing', () => {
    ...
    const moved = encounters.transitionEncounter(
  ...
  it('hydrates persisted encounters and timelines and caches async timeline reads', async () => {
    ...
    expect(encounters.listAll('acc_repo' as never)).toHaveLength(1);
    expect(encounters.listTimeline('acc_repo' as never, 'enc_repo_1' as never)).toHaveLength(1);

    const firstAsync = await encounters.listTimelineAsync('acc_repo' as never, 'enc_repo_1' as never);
    const secondAsync = await encounters.listTimelineAsync('acc_repo' as never, 'enc_repo_1' as never);
    ...
    await expect(
      encounters.listTimelineAsync('acc_repo' as never, 'enc_missing' as never)
