...
function normalizeStatus(status: string): ClinicalNoteStatus {
  return status === 'signed' ? 'signed' : 'draft';
}

function normalizeType(type: string): ClinicalNoteType {
  return type === 'SOAP' ? 'SOAP' : 'SOAP';
}

function mapClinicalNoteRow(row: Record<string, unknown>): ClinicalNoteRecord {
  ...
  return {
    ...
    soap: typeof soapRaw === 'object' && soapRaw !== null ? (soapRaw as SoapDto) : null
  };
}

async function queryNoteById(
  queryable: {
  ...
  return mapClinicalNoteRow(result.rows[0] as Record<string, unknown>);
}

export function createClinicalNotesRepo(db: DbClient): ClinicalNotesRepo {
  return {
    async findEncounterInAccount(accountId: string, encounterId: string): Promise<boolean> {
    ...
    async createDraft(input): Promise<ClinicalNoteRecord> {
      ...
      try {
        ...
        const noteId = String((noteResult.rows[0] as Record<string, unknown>).id);
    ...
    async findById(accountId: string, noteId: string): Promise<ClinicalNoteRecord | null> {
      return queryNoteById(db.$client, accountId, noteId);
    },

    async updateDraft(input): Promise<ClinicalNoteRecord | null> {
      const client = await db.$client.connect();
    ...
    async signDraftById(input): Promise<ClinicalNoteRecord | null> {
      const result = await db.$client.query(
        `
          update clinical_notes n
          set
