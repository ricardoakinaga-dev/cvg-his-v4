...
describe('Notification Repository', () => {
  ...
  describe('createNotificationTemplate', () => {
    it('cria template corretamente', async () => {
      ...
      mockDb.$client.query.mockResolvedValueOnce({ rows: [mockRow] });
      ...
      expect(mockDb.$client.query).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('listNotificationTemplates', () => {
    it('lista templates com paginação', async () => {
      // Mock: count query
      mockDb.$client.query.mockResolvedValueOnce({ rows: [{ count: '10' }] });
