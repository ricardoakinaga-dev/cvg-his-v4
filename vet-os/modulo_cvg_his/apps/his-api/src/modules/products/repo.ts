...
type UpdateProductInput = Partial<Omit<CreateProductInput, 'accountId'>>;
...
export function createProductsRepo(db: DbClient) {
  return {
    async create(input: CreateProductInput): Promise<ProductRecord> {
      ...
      return mapProductRow(queryResult.rows[0] as Record<string, unknown>);
    },

    async findById(accountId: string, id: string): Promise<ProductRecord | null> {
      const queryResult = await db.$client.query('select * from products where id = $1 and account_id = $2 limit 1', [id, accountId]);
    ...
    async findByCode(accountId: string, code: string): Promise<ProductRecord | null> {
      const queryResult = await db.$client.query('select * from products where account_id = $1 and code = $2 limit 1', [accountId, code]);
      if (queryResult.rows.length === 0) {
        return null;
      }
      return mapProductRow(queryResult.rows[0] as Record<string, unknown>);
    ...
    async updateById(accountId: string, id: string, patch: UpdateProductInput): Promise<ProductRecord | null> {
      const fields: string[] = [];
    ...
    async list(input: ListProductsInput) {
      ...
      const [rowsResult, totalResult] = await Promise.all([
        ...
        db.$client.query(`select count(*)::int as total from products where ${whereClause}`, values)
      ]);

      return {
        data: rowsResult.rows.map((row) => mapProductRow(row as Record<string, unknown>)),
        page: input.page,
