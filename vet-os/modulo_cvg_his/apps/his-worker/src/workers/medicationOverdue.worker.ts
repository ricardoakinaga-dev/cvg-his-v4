...
type SlotAdministrationStatus = 'administered' | 'refused' | 'delayed' | 'held';

type SlotAdministrationRow = {
  status: SlotAdministrationStatus;
  delayedUntil: Date | null;
};

function parseScheduleType(value: unknown): 'interval' | 'fixed_times' {
...
function mapOrderRow(row: Record<string, unknown>): OverdueOrderRow {
  return {
...
function calculateSeverity(delayMinutes: number): 'low' | 'medium' | 'high' {
  if (delayMinutes >= 120) {
    return 'high';
  }
...
function buildAlertMessage(row: OverdueOrderRow, scheduledFor: Date, delayMinutes: number): string {
  return `Dose overdue: ${row.medicationName} for ${row.patientName} (slot ${scheduledFor.toISOString()}, delay ${delayMinutes}m)`;
}

function shouldUpgradeSeverity(
  currentSeverity: 'low' | 'medium' | 'high',
  nextSeverity: 'low' | 'medium' | 'high'
): boolean {
...
function mapAdministrationStatus(value: unknown): SlotAdministrationStatus {
...
async function findAdministrationForSlot(input: {
  ...
  orderId: string;
  scheduledFor: Date;
}): Promise<SlotAdministrationRow | null> {
...
function mapAlertSeverity(value: unknown): 'low' | 'medium' | 'high' {
  const raw = String(value);
  if (raw === 'high') {
    return 'high';
  }
...
async function loadOverdueCandidates(accountId?: string): Promise<OverdueOrderRow[]> {
  const values: string[] = [];
  let accountFilter = '';
...
function logInfo(context: WorkerLogContext, message: string, extra: Record<string, unknown> = {}): void {
  console.info(
    JSON.stringify({
      level: 'info',
      message,
...
export function createMedicationOverdueWorker(
  connection: Redis,
  prefix: string
): Worker<MedicationOverdueScanJobData, MedicationOverdueScanJobResult, MedicationOverdueScanJobName> {
  return new Worker<
  ...
  >(
    ...
    async (job) => {
      ...
      const context: WorkerLogContext = {
      ...
      const now = new Date();
      const graceMinutes =
        Number.isFinite(job.data.graceMinutes) && (job.data.graceMinutes ?? 0) > 0
