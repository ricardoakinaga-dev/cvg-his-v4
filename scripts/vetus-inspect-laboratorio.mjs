...
const LEGACY_PAGES = [
  { slug: 'exames', url: 'https://erp.vetus.com.br/Sistema/Laboratorio/Exames.htm' },
  { slug: 'laudos', url: 'https://erp.vetus.com.br/Sistema/Laboratorio/Laudos.htm' },
  { slug: 'tipos-laudo', url: 'https://erp.vetus.com.br/Sistema/Laboratorio/TiposDeLaudo.htm' }
];
const ROOT_STAMP = new Date().toISOString().replace(/[:.]/g, '-');
const ROOT_DIR = path.resolve('docs/vetus/inspection', `${ROOT_STAMP}-laboratorio`);
...
async function main() {
  ...
  for (const legacyPage of LEGACY_PAGES) {
    ...
    await dump(page, `laboratorio-${legacyPage.slug}`);
