...
export function checkCriticalCoverage({ root, manifestPath, artifactsPath, head }) {
  ...
  if (!manifest.files?.length || !manifest.components?.length || !manifest.requiredShards?.length)
    throw new Error('Critical manifest must contain files, components and required shards');
  if (new Set(manifest.files.map((file) => file.path)).size !== manifest.files.length)
    errors.push('duplicate manifest source paths');
  // SQL migrations have no JavaScript executable counters. They are verified
  // by the specialized database evidence consumer below and must never enter
  // the Istanbul merge, even if an invalid manifest labels one as JS.
  const applicable = manifest.files.filter(
    (file) => file.applicability === 'javascript-metrics' && !file.path.endsWith('.sql')
  );
  ...
  let specializedVue = { status: 'PASS', errors: [], acceptedPaths: [] };
  if (pendingVue.length) {
    ...
    if (!contractPath.ok) {
      specializedVue = {
        status: 'FAIL',
        errors: [`Vue specialized contract unavailable: ${contractPath.reason}`],
        acceptedPaths: []
      };
    } else {
      const contractBytes = readFileSync(contractPath.realpath);
      let contract;
      try {
        ...
        specializedVue = {
          status: 'FAIL',
          errors: [`invalid Vue specialized contract JSON: ${error.message}`],
          acceptedPaths: []
        };
      }
      if (contract) {
        const evidencePath = artifactsRoot.ok
          ? resolve(artifactsRoot.absolute, 'vue-specialized/evidence.json')
          : resolve(root, 'invalid-artifacts-root/vue-specialized/evidence.json');
        specializedVue = checkVueSpecializedEvidence({
          root,
          manifest,
          contract,
          manifestBytes: bytes,
  ...
  for (const file of manifest.files) {
    ...
    if (file.applicability === 'javascript-metrics' && sourcePath.ok) {
      if (expectedCoveragePaths.has(sourcePath.path))
        errors.push(`duplicate normalized source path: ${file.path}`);
      expectedCoveragePaths.add(sourcePath.path);
      sourceIdentities.set(file.path, sourcePath.path);
    }
    const isSqlSource = typeof file.path === 'string' && file.path.endsWith('.sql');
    if (isSqlSource && file.applicability === 'javascript-metrics')
      errors.push(`SQL migration cannot use JavaScript coverage applicability: ${file.path}`);
    if (
      file.applicability !== 'javascript-metrics' &&
      !isSqlSource &&
      !(file.path.endsWith('.vue') && specializedVue.acceptedPaths.includes(file.path))
    )
      errors.push(
        `unresolved applicability requires specialized evidence and independent review: ${file.path}`
      );
  }
  const sqlEvidence = verifySqlMigrationEvidence({
    root,
    manifestPath
  });
  if (sqlEvidence.status === 'FAIL')
    errors.push(...sqlEvidence.errors.map((error) => `SQL evidence: ${error}`));
  for (const shard of artifactsRoot.ok ? manifest.requiredShards : []) {
    ...
    const metadataPath = shardPath.ok
      ? resolveContainedPath(shardPath.absolute, 'shard.json')
      : shardPath;
    if (!metadataPath.ok) {
      errors.push(
        metadataPath.reason.includes('escapes root')
          ? `shard path escapes artifacts root: ${shard}`
          : `missing shard: ${shard}`
      );
      continue;
    }
    try {
      ...
      if (
        ...
        errors.push(`shard not finalized after successful exit: ${shard}`);
      ...
      for (const path of manifest.executionInputs ?? []) {
        ...
        if (!inputPath.ok) {
          errors.push(
            inputPath.reason.includes('escapes root')
              ? `execution input path escapes root: ${shard}:${path}`
              : `execution input unavailable: ${shard}:${path}`
          );
          continue;
        }
        if (metadata.executionInputHashes?.[path] !== hash(readFileSync(inputPath.realpath)))
          errors.push(`execution input hash mismatch: ${shard}:${path}`);
      }
      for (const file of applicable)
        if (metadata.sourceHashes?.[file.path] !== file.sha256)
          errors.push(`shard source hash mismatch: ${shard}:${file.path}`);
      ...
      if (metadata.coverageSha256 !== hash(coverageBytes))
        errors.push(`coverage digest mismatch: ${shard}`);
      const testResultPath = resolveContainedPath(
        shardPath.absolute,
        metadata.testResultFile ?? 'missing-test-result'
      );
      ...
      if (metadata.testResultSha256 !== hash(testResultBytes))
        errors.push(`test result digest mismatch: ${shard}`);
      const testResult = JSON.parse(testResultBytes);
      if (
        testResult.runId !== metadata.runId ||
        testResult.shard !== shard ||
        testResult.status !== 'passed'
      )
        errors.push(`invalid test result: ${shard}`);
      const raw = JSON.parse(coverageBytes);
      if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        throw new Error('invalid coverage report schema');
      const normalized = {};
      let malformed = false;
      ...
      for (const [path, entry] of Object.entries(raw)) {
        ...
        if (!sourcePath.ok) {
          errors.push(
            `${sourcePath.reason.includes('escapes root') ? 'raw coverage path escapes root' : 'raw coverage source unavailable'}: ${shard}:${path}`
          );
          malformed = true;
          continue;
        }
        const key = sourcePath.path;
        if (!expectedCoveragePaths.has(key)) {
          errors.push(`unexpected raw coverage source: ${shard}:${key}`);
          malformed = true;
          continue;
        }
        if (seenCoveragePaths.has(key)) {
          errors.push(`duplicate raw coverage source: ${shard}:${key}`);
          malformed = true;
          continue;
        }
        seenCoveragePaths.add(key);
        const rawErrors = validateRawCoverageEntry(
          entry,
          readFileSync(sourcePath.realpath, 'utf8')
        );
        if (typeof entry?.path !== 'string') rawErrors.push('coverage path/key mismatch');
          ...
          if (!entryPath.ok)
            rawErrors.push(
              entryPath.reason.includes('escapes root')
                ? 'coverage source path escapes root'
                : 'coverage source path unavailable'
            );
        ...
        if (rawErrors.length) {
          malformed = true;
          errors.push(
            ...rawErrors.map((error) => `malformed raw coverage ${shard}:${key}: ${error}`)
          );
        }
  ...
  for (const file of applicable)
    if (!merged.files().includes(sourceIdentities.get(file.path)))
      errors.push(`missing instrumented file: ${file.path}`);
  for (const file of applicable) {
    ...
    for (const metric of ['s', 'f', 'b']) {
      ...
      if (counts.some((count) => !Number.isSafeInteger(count) || count < 0))
        errors.push(`unsafe merged coverage counters: ${file.path}:${metric}`);
    }
    for (const [map, counts] of [
      ...
      if (Object.keys(data[map]).some((key) => !(key in data[counts])))
        errors.push(`incomplete instrumentation counters: ${file.path}:${map}`);
    }
    const sourcePath = resolveContainedPath(root, file.path);
    if (!sourcePath.ok) continue;
    const source = readFileSync(sourcePath.realpath, 'utf8');
    if (!Object.keys(data.statementMap).length) {
      const hasCounters = [data.s, data.f, data.b].some((metric) => Object.keys(metric).length);
      if (isTypeOnlySource(source)) {
        if (hasCounters)
          errors.push(`type-only source has runtime coverage counters: ${file.path}`);
        else
          typeOnlySources.push({
            path: file.path,
            sourceSha256: hash(source),
            reason: 'type-only-source'
          });
      } else if (!hasCounters && isRuntimeReexportOnly(source))
        metriclessReexports.push({
          path: file.path,
          sourceSha256: hash(source),
          reason: 'runtime-reexports-only'
        });
    ...
    visit(ast);
    if (hasBranch && !Object.keys(data.branchMap).length)
      errors.push(`executable branches lack instrumentation: ${file.path}`);
    for (const [key, branch] of Object.entries(data.branchMap)) {
      if (!Array.isArray(data.b[key]) || data.b[key].length !== branch.locations.length)
        errors.push(`incomplete branch instrumentation: ${file.path}:${key}`);
    }
    if (hasFunction && !Object.keys(data.fnMap).length)
      errors.push(`executable functions lack instrumentation: ${file.path}`);
  }
  const components = {};
  for (const component of manifest.components) {
    ...
    for (const file of applicable.filter((file) => file.components.includes(component))) {
      const identity = sourceIdentities.get(file.path);
      if (merged.files().includes(identity))
        summary.merge(merged.fileCoverageFor(identity).toSummary());
    }
    components[component] = summary.toJSON();
    for (const metric of ['lines', 'statements', 'functions', 'branches']) {
      const value = summary[metric];
      if (!Number.isFinite(manifest.thresholds[metric]) || manifest.thresholds[metric] < 85)
        errors.push(`invalid frozen threshold: ${metric}`);
      if (
        !value.total ||
        !Number.isFinite(value.pct) ||
        value.pct < Math.max(85, manifest.thresholds[metric])
      )
        errors.push(
          `critical metric below threshold or unmeasured: ${component}:${metric}=${value.pct}`
        );
    }
  }
  return {
