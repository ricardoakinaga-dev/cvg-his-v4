...
const ROOT_DIR = path.resolve('docs/vetus/inspection', `${ROOT_STAMP}-comandas`);
...
async function main() {
  ...
  await page.goto('https://erp-beta.vetus.com.br/comandas', { waitUntil: 'domcontentloaded', timeout: 60_000 });
  await page.waitForTimeout(2500);
  await dump(page, 'comandas-lista');

  await clickMaybe(page, 'main >> text="Informações do cliente"');
  await clickMaybe(page, 'main >> text="Serviços / Produtos"');
  await dump(page, 'comandas-lista-expandida');

  const beforeDetailReq = requests.length;
  const beforeDetailRes = responses.length;

  await page.getByText(/ver comanda/i).first().click();
  await page.waitForLoadState('domcontentloaded', { timeout: 30_000 }).catch(() => {});
  await page.waitForTimeout(3000);
  await dump(page, 'comandas-detalhe');

  const selectors = [
    'main >> text="Informações do cliente"',
    'main >> text="Serviços / Produtos"',
    'main >> text="Animais Vinculados na Comanda"',
    'main >> text="Ver Cadastro do Cliente"',
    'main >> text="Finalizar Comanda"'
  ];
  for (const selector of selectors) {
    await clickMaybe(page, selector);
  }

  await dump(page, 'comandas-detalhe-expandido');
