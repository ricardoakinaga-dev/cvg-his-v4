...
const LEGACY_QUOTES_URL = 'https://erp.vetus.com.br/Sistema/Atendimento/Orcamentos.htm';
const ROOT_STAMP = new Date().toISOString().replace(/[:.]/g, '-');
const ROOT_DIR = path.resolve('docs/vetus/inspection', `${ROOT_STAMP}-orcamentos`);
...
async function goToLegacy(page, credentials) {
  try {
    await page.goto(LEGACY_QUOTES_URL, { waitUntil: 'domcontentloaded', timeout: 60_000 });
  } catch {
    // continue
  }
...
async function openFirstDetail(page) {
  const candidates = [
    page.locator('a').filter({ hasText: /abrir|alterar|editar|consultar|visualizar|detalhes/i }),
    page.locator('img[title*="Abrir"], img[title*="Alterar"], img[title*="Editar"]'),
    page.getByText(/abrir/i)
  ];
...
async function main() {
  ...
  await dumpState(page, 'estado-orcamentos-legacy');
  await dump(page, 'orcamentos-lista');

  await clickMaybe(page, 'text="Pesquisar"');
  await dump(page, 'orcamentos-lista-pos-pesquisa');
  ...
  if (detailOpened) {
    ...
    await dumpState(page, 'estado-orcamentos-detalhe');
    await dump(page, 'orcamentos-detalhe');
  }
