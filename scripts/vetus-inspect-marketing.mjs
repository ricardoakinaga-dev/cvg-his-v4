...
const LEGACY_PAGES = [
  { slug: 'sms-simples', url: 'https://erp.vetus.com.br/Sistema/Marketing/SMSSimples.htm' },
  { slug: 'sms-campanhas', url: 'https://erp.vetus.com.br/Sistema/Marketing/SMSCampanhaP.htm' },
  { slug: 'layout-email-vacina', url: 'https://erp.vetus.com.br/Sistema/Vacina/VacinaLayoutEmail.htm' },
  { slug: 'sms-configuracao', url: 'https://erp.vetus.com.br/Sistema/Marketing/SMSConfiguracao.htm' }
];
const ROOT_STAMP = new Date().toISOString().replace(/[:.]/g, '-');
const ROOT_DIR = path.resolve('docs/vetus/inspection', `${ROOT_STAMP}-marketing`);
...
async function main() {
  ...
  for (const legacyPage of LEGACY_PAGES) {
    ...
    await dump(page, `marketing-${legacyPage.slug}`);
