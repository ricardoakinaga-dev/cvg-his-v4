...
const BETA_FINANCE_URL = 'https://erp-beta.vetus.com.br/dashboard-financeiro';
const LEGACY_PAGES = [
  { slug: 'gaveta', url: 'https://erp.vetus.com.br/Sistema/Financeiro/Gaveta.htm' },
  { slug: 'contas-receber', url: 'https://erp.vetus.com.br/Sistema/Financeiro/ContasAReceber.htm' },
  { slug: 'contas-pagar', url: 'https://erp.vetus.com.br/Sistema/Financeiro/ContasAPagar.htm' },
  { slug: 'transacoes', url: 'https://erp.vetus.com.br/Sistema/Financeiro/Transacoes.htm' },
  { slug: 'formas-pagamento', url: 'https://erp.vetus.com.br/Sistema/Cadastros/FormasDePagamento.htm' }
];
const ROOT_STAMP = new Date().toISOString().replace(/[:.]/g, '-');
const ROOT_DIR = path.resolve('docs/vetus/inspection', `${ROOT_STAMP}-financeiro`);
...
async function main() {
  ...
  const betaReqStart = requests.length;
  const betaResStart = responses.length;
  await goToUrl(page, BETA_FINANCE_URL, credentials);
  await dump(page, 'financeiro-beta-dashboard');
  const betaReqEnd = requests.length;
  const betaResEnd = responses.length;
  ...
  for (const legacyPage of LEGACY_PAGES) {
    ...
    await dumpState(page, `estado-${legacyPage.slug}`);
    await dump(page, `financeiro-legacy-${legacyPage.slug}`);
  ...
  const artifact = {
    finalUrl: page.url(),
    betaRequests: requests.slice(betaReqStart, betaReqEnd),
    betaResponses: responses.slice(betaResStart, betaResEnd),
    legacyArtifacts,
