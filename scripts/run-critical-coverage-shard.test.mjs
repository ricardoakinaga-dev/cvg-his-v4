...
function fixture({ coverageBytes, resultBytes, beforeInputs, afterInputs } = {}) {
  ...
  const manifestBytes = Buffer.from(
    JSON.stringify({ files: [{ path: 'a.ts', applicability: 'javascript-metrics' }] })
  );
  ...
  const defaultResultBytes = Buffer.from(
    JSON.stringify({ runId: 'run', shard: 'unit', status: 'passed' })
  );
...
function cliFixture() {
  ...
  writeFileSync(
    join(root, 'docs/engineering/critical-coverage-scope.json'),
    JSON.stringify({
  ...
  writeFileSync(
    ...
    [
      ...
      "if (process.env.STUB_CANDIDATE_PUBLICATION_TARGET) fs.symlinkSync(process.env.STUB_CANDIDATE_PUBLICATION_TARGET, path.join(output, 'shard.json'));"
    ].join('\n')
  );
...
for (const [name, counts] of [
  ...
  [
    'unsafe branch counter',
    { statement: 0, function: 0, branches: [1, Number.MAX_SAFE_INTEGER + 1] }
  ]
])
  test(`finalizer rejects ${name}`, () => {
    ...
    try {
      ...
      assert.ok(
        result.errors.some((error) => error.includes('invalid hit count')),
        result.errors.join('\n')
      );
      assert.equal(result.coverageSha256, sha256(coverageBytes));
    } finally {
...
test('finalizer rejects source locations outside the real source', () => {
  ...
  try {
    ...
    assert.equal(result.status, 'failed');
    assert.ok(
      result.errors.some((error) => error.includes('invalid source location')),
      result.errors.join('\n')
    );
  } finally {
    x.cleanup();
...
test('finalizer rejects malformed coverage JSON without throwing and retains its digest', () => {
  ...
  try {
    ...
    assert.equal(result.status, 'failed');
    assert.ok(
      result.errors.some((error) => error.includes('invalid coverage JSON')),
      result.errors.join('\n')
    );
    assert.equal(result.coverageSha256, sha256(coverageBytes));
  } finally {
...
for (const serialized of ['null', '[]', '{"a.ts":null}'])
  test(`finalizer rejects coverage schema ${serialized}`, () => {
    ...
    try {
      ...
      assert.ok(
        result.errors.some((error) =>
          /invalid coverage report schema|invalid file coverage schema/.test(error)
        ),
        result.errors.join('\n')
      );
    } finally {
...
test('finalizer rejects an empty coverage map instead of certifying by existence', () => {
  ...
  try {
    ...
    assert.equal(result.status, 'failed');
    assert.ok(
      result.errors.some((error) => error.includes('empty report')),
      result.errors.join('\n')
    );
    assert.equal(result.coverageSha256, sha256(coverageBytes));
  } finally {
...
test('containment rejects outside frozen inputs and report symlinks before content reads', () => {
  ...
  try {
    ...
    assert.throws(
      () => snapshotInputs(root, { executionInputs: [outsideSource] }),
      /path escapes root/
    );
    assert.throws(
      () => snapshotInputs(root, { executionInputs: [relative(root, outsideSource)] }),
      /path escapes root/
    );
    symlinkSync(outsideSource, join(root, 'linked-input.ts'));
    assert.throws(
      () => snapshotInputs(root, { executionInputs: ['linked-input.ts'] }),
      /path escapes root/
    );
    ...
    assert.ok(
      result.errors.some((error) =>
        error.includes('coverage output path invalid: path escapes root via symlink')
      ),
      result.errors.join('\n')
    );
    ...
    assert.ok(
      result.errors.some((error) =>
        error.includes('test result path invalid: path escapes root via symlink')
      ),
      result.errors.join('\n')
    );
    ...
    assert.ok(
      result.errors.some((error) =>
        error.includes('execution input path escapes root: unit:linked-input.ts')
      ),
      result.errors.join('\n')
    );
    ...
    assert.equal(result.status, 'FAIL');
    assert.ok(
      result.errors.some((error) => error.includes('coverage path escapes root via symlink')),
      result.errors.join('\n')
    );
    ...
    assert.equal(result.status, 'FAIL');
    assert.ok(
      result.errors.some((error) => error.includes('test result path escapes root via symlink')),
      result.errors.join('\n')
    );
  } finally {
    finalizer.cleanup();
...
test('CLI guards artifact parent and candidate publication symlinks', () => {
  ...
  const candidatePublication = cliFixture();
  const candidateOutside = mkdtempSync(join(tmpdir(), 'coverage-cli-candidate-outside-'));
  try {
    const parentSentinel = join(parentOutside, 'sentinel.txt');
    writeFileSync(parentSentinel, 'parent sentinel');
    mkdirSync(join(parentSymlink.root, 'artifacts/consolidacao-2026-09-05/coverage-scope'), {
      recursive: true
    });
    ...
    const previousSentinel = join(candidateOutside, 'previous-shard.json');
    ...
    assert.equal(run.status, 1, run.stderr);
    assert.doesNotMatch(run.stderr, /previous shard/);
    assert.equal(readFileSync(previousSentinel, 'utf8'), 'previous sentinel');

    const candidateSentinel = join(candidateOutside, 'candidate-shard.json');
    writeFileSync(candidateSentinel, 'candidate sentinel');
    run = candidatePublication.run({ STUB_CANDIDATE_PUBLICATION_TARGET: candidateSentinel });
    assert.equal(run.status, 1, run.stderr);
    assert.match(run.stderr, /candidate shard .*path escapes root via symlink/);
    assert.equal(readFileSync(candidateSentinel, 'utf8'), 'candidate sentinel');
  } finally {
    parentSymlink.cleanup();
    previousShard.cleanup();
    candidatePublication.cleanup();
    rmSync(parentOutside, { recursive: true, force: true });
    rmSync(candidateOutside, { recursive: true, force: true });
  }
});
...
test('finalizer rejects escaping and unexpected source identities without reading outside root', () => {
  ...
  try {
    ...
    assert.equal(result.status, 'failed');
    assert.ok(
      result.errors.some((error) => error.includes('raw coverage path escapes root')),
      result.errors.join('\n')
    );
    ...
    assert.equal(result.status, 'failed');
    assert.ok(
      result.errors.some((error) => error.includes('unexpected raw coverage source')),
      result.errors.join('\n')
    );
    ...
    writeFileSync(
      join(x.root, 'coverage-final.json'),
      Buffer.from(`${JSON.stringify(outsideEntry)}\n`)
    );
    result = x.finalize();
    assert.equal(result.status, 'failed');
    assert.ok(
      result.errors.some((error) => error.includes('coverage source path escapes root')),
      result.errors.join('\n')
    );
  } finally {
    x.cleanup();
  ...
  test(`post-exit finalizer ${kind}`, () => {
    ...
    try {
      ...
      if (kind !== 'missing-output')
        writeFileSync(join(root, 'coverage-final.json'), coverageBytes);
      writeFileSync(
        join(root, 'test-result.json'),
        JSON.stringify({
          runId: kind === 'foreign-result' ? 'other' : 'run',
          shard: 'unit',
          status: 'passed'
        })
      );
...
test('critical shard classification routes database suites to integration', async () => {
  const { isCriticalIntegrationTest, shardExecutionEnv } =
    await import('./lib/critical-shard-classification.mjs');
  assert.equal(isCriticalIntegrationTest('tests/integration/database/migration.test.ts'), true);
  assert.equal(
    isCriticalIntegrationTest(
      'packages/modules/diagnostics/src/laboratory-postgres.integration.test.ts'
    ),
    true
  );
  assert.equal(isCriticalIntegrationTest('tests/unit/api/auth-dispatch-contract.test.ts'), false);
  assert.equal(isCriticalIntegrationTest('packages/modules/auth/src/oidc.test.ts'), false);
  assert.equal(isCriticalIntegrationTest('packages/modules/ml/src/ml.test.ts'), true);
  assert.equal(isCriticalIntegrationTest(''), false);
  assert.equal(isCriticalIntegrationTest(undefined), false);

  assert.deepEqual(shardExecutionEnv('vitest-unit', { A: '1' }), { A: '1' });
  assert.deepEqual(shardExecutionEnv('vitest-integration', { A: '1' }), {
    A: '1',
    REQUIRE_TEST_DB: '1'
  });
  assert.throws(() => shardExecutionEnv('native-api', {}), /Unsupported critical Vitest shard/);
});
