...
const BETA_PAGES = [
  { slug: 'produtos', url: 'https://erp-beta.vetus.com.br/produtos' },
  { slug: 'estoques', url: 'https://erp-beta.vetus.com.br/estoques' },
  { slug: 'fornecedores-despesas', url: 'https://erp-beta.vetus.com.br/fornecedores-e-despesas' },
  { slug: 'fabricantes', url: 'https://erp-beta.vetus.com.br/fabricantes' },
  { slug: 'grupos-produto', url: 'https://erp-beta.vetus.com.br/grupos-de-produto' }
];
const LEGACY_PAGES = [
  { slug: 'consulta-precos', url: 'https://erp.vetus.com.br/Sistema/Estoque/ConsultaDePrecos.htm' },
  { slug: 'entrada-nota-fiscal', url: 'https://erp.vetus.com.br/Sistema/Estoque/EntradaNotaFiscal.htm' },
  { slug: 'transacao-estoque', url: 'https://erp.vetus.com.br/Sistema/Estoque/TransacaoNoEstoque.htm' },
  { slug: 'transferencia-estoques', url: 'https://erp.vetus.com.br/Sistema/Estoque/TransferenciaEntreEstoques.htm' },
  { slug: 'compras', url: 'https://erp.vetus.com.br/Sistema/Estoque/Compras.htm' }
];
const ROOT_STAMP = new Date().toISOString().replace(/[:.]/g, '-');
const ROOT_DIR = path.resolve('docs/vetus/inspection', `${ROOT_STAMP}-estoque`);
...
async function openFirstBetaDetail(page) {
  const candidates = [
    page.getByText(/ver|visualizar|detalhe|detalhes|abrir/i),
    page.locator('main a').filter({ hasText: /editar|abrir|detalhe|visualizar/i }),
    page.locator('tbody tr a'),
    page.locator('[data-testid*="row"] a')
  ];
...
async function main() {
  ...
  const betaArtifacts = [];
  for (const betaPage of BETA_PAGES) {
    const reqStart = requests.length;
    const resStart = responses.length;

    await goToUrl(page, betaPage.url, credentials);
    await dumpState(page, `estado-beta-${betaPage.slug}`);
    await dump(page, `estoque-beta-${betaPage.slug}`);

    let detailOpened = false;
    if (betaPage.slug === 'produtos') {
      detailOpened = await openFirstBetaDetail(page).catch(() => false);
      if (detailOpened) {
        await page.waitForLoadState('domcontentloaded', { timeout: 30_000 }).catch(() => {});
        await page.waitForTimeout(3000);
        await dumpState(page, 'estado-beta-produtos-detalhe');
        await dump(page, 'estoque-beta-produtos-detalhe');
      }
    }
    ...
    betaArtifacts.push({
      ...
      finalUrl: page.url(),
      detailOpened,
      requests: requests.slice(reqStart),
      responses: responses.slice(resStart)
    });
  }
  ...
  for (const legacyPage of LEGACY_PAGES) {
    ...
    await goToUrl(page, legacyPage.url, credentials);
    await dumpState(page, `estado-legacy-${legacyPage.slug}`);
    await dump(page, `estoque-legacy-${legacyPage.slug}`);
