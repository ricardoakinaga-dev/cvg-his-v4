...
const LEGACY_URL = 'https://erp.vetus.com.br/Sistema/Atendimento/EsteiraExames.htm';
const ROOT_STAMP = new Date().toISOString().replace(/[:.]/g, '-');
const ROOT_DIR = path.resolve('docs/vetus/inspection', `${ROOT_STAMP}-esteira-exames`);
...
async function main() {
  ...
  await goToUrl(page, LEGACY_URL, credentials);
  await dumpState(page, 'estado-esteira-exames');
  await dump(page, 'esteira-exames-lista');

  for (const action of ['Pesquisar', 'Solicitado', 'Coletado', 'Laudado', 'Entregue']) {
    const before = page.url();
    const clicked = await clickIfVisible(page, action);
    if (!clicked) continue;
    extraActions.push({ action, beforeUrl: before, afterUrl: page.url() });
    await dump(page, `esteira-exames-${action.toLowerCase().replace(/[^a-z0-9]+/gi, '-')}`);
  }
