...
const LEGACY_POINTS_URL = 'https://erp.vetus.com.br/Sistema/Atendimento/PontuacaoResgate.htm';
const ROOT_STAMP = new Date().toISOString().replace(/[:.]/g, '-');
const ROOT_DIR = path.resolve('docs/vetus/inspection', `${ROOT_STAMP}-pontos`);
...
async function goToLegacy(page, credentials) {
  try {
    await page.goto(LEGACY_POINTS_URL, { waitUntil: 'domcontentloaded', timeout: 60_000 });
  } catch {
    // continue
  }
...
async function main() {
  ...
  await dumpState(page, 'estado-pontos-legacy');
  await dump(page, 'pontos-lista');

  const extraActions = [];
  for (const selector of ['text="Pesquisar"', 'text="Resgatar"', 'text="Cliente"', 'text="Pontos"']) {
    const clicked = await clickMaybe(page, selector);
    if (clicked) {
      extraActions.push(selector);
      await dump(page, `pontos-${selector.replace(/[^a-zA-Z0-9]+/g, '-').toLowerCase()}`);
    }
  }
