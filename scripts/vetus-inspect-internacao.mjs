...
const LEGACY_HOSPITALIZATION_URL = 'https://erp.vetus.com.br/Sistema/Internacao/Internacao.htm';
const ROOT_STAMP = new Date().toISOString().replace(/[:.]/g, '-');
const ROOT_DIR = path.resolve('docs/vetus/inspection', `${ROOT_STAMP}-internacao`);
...
async function main() {
  ...
  await goToUrl(page, LEGACY_HOSPITALIZATION_URL, credentials);
  await dumpState(page, 'estado-internacao-legacy');
  await dump(page, 'internacao-lista');

  for (const action of ['Pesquisar', 'Incluir', 'Entrada', 'Internação']) {
    const before = page.url();
    const clicked = await clickIfVisible(page, action);
    if (!clicked) continue;
    extraActions.push({ action, beforeUrl: before, afterUrl: page.url() });
    await dump(page, `internacao-${action.toLowerCase().replace(/[^a-z0-9]+/gi, '-')}`);
  }
