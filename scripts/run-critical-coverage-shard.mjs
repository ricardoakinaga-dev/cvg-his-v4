...
import { shardExecutionEnv } from './lib/critical-shard-classification.mjs';
...
export function snapshotInputs(root, manifest) {
  ...
  return Object.fromEntries(
    manifest.executionInputs.map((path) => {
...
export function ensureContainedDirectory(root, directory) {
  ...
  if (existing.reason !== 'path does not exist')
    throw artifactError('artifact directory', directory, existing.reason);

  const rootPath = resolve(root);
  const directoryPath = resolve(directory);
  const relativePath = relative(rootPath, directoryPath);
  if (!relativePath || relativePath === '..' || relativePath.startsWith(`..${sep}`))
    throw artifactError('artifact directory', directory, 'path escapes root');
  ...
  for (const part of relativePath.split(sep)) {
    ...
    try {
      ...
      if (entry.isSymbolicLink())
        throw artifactError('artifact directory', current, 'path escapes root via symlink');
      if (!entry.isDirectory())
        throw artifactError('artifact directory', current, 'path is not a directory');
    } catch (error) {
...
const expectedCoveragePaths = (root, manifest, errors) => {
  ...
  for (const file of manifest.files) {
    ...
    if (expected.has(sourcePath.path))
      errors.push(`duplicate manifest source path: ${sourcePath.path}`);
    expected.add(sourcePath.path);
  }
  return expected;
};

export function finalizeShard({
  root,
  manifestBytes,
  beforeInputs,
  afterInputs,
  ...
  const resultPath = outputPath.ok
    ? resolveContainedPath(outputPath.absolute, 'test-result.json')
    : outputPath;
  const coveragePath = outputPath.ok
    ? resolveContainedPath(outputPath.absolute, 'coverage-final.json')
    : outputPath;
  if (JSON.stringify(beforeInputs) !== JSON.stringify(afterInputs) || headBefore !== headAfter)
    errors.push('executed candidate changed during run');
  ...
  if (!result || result.runId !== runId || result.shard !== shard || result.status !== 'passed')
    errors.push('test result incomplete or missing');
  ...
  return {
    ...
    executionInputHashes: beforeInputs,
    sourceHashes: Object.fromEntries(
      manifestFiles
        .filter((f) => f?.applicability === 'javascript-metrics')
        .map((f) => [f.path, beforeInputs[f.path]])
    ),
    coverageFile: `${runId}/coverage-final.json`,
    coverageSha256: coverageBytes ? sha256(coverageBytes) : null,
    testResultFile: `${runId}/test-result.json`,
    testResultSha256: resultBytes ? sha256(resultBytes) : null
  };
}
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  const root = process.cwd();
  const shard = process.argv[2];
  if (!['vitest-unit', 'vitest-integration'].includes(shard))
    throw new Error(
      'Usage: node scripts/run-critical-coverage-shard.mjs vitest-unit|vitest-integration [test filters]'
    );
  const manifestBytes = readFileSync(
    resolve(root, 'docs/engineering/critical-coverage-scope.json')
  );
  ...
  const parentPath = ensureContainedDirectory(
    root,
    resolve(root, 'artifacts/consolidacao-2026-09-05/coverage-scope', shard)
  );
  ...
  const invocationPath = writableContainedArtifact(
    output,
    'invocation.json',
    'invocation artifact'
  );
  writeFileSync(
    invocationPath.realpath,
    JSON.stringify({ command: ['pnpm', ...command], head: headBefore, beforeInputs }, null, 2)
  );
  const child = spawnSync('pnpm', command, {
    cwd: root,
    stdio: 'inherit',
    env: shardExecutionEnv(shard, {
      ...process.env,
      CRITICAL_COVERAGE_SHARD: shard,
      CRITICAL_COVERAGE_RUN_ID: runId
    })
  });
  ...
  const candidatePublication = writableContainedArtifact(output, 'shard.json', 'candidate shard');
  writeFileSync(candidatePublication.realpath, JSON.stringify(metadata, null, 2));
