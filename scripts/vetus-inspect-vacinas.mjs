...
const LEGACY_VACCINES_URL = 'https://erp.vetus.com.br/Sistema/Vacina/Vacinas.htm';
const ROOT_STAMP = new Date().toISOString().replace(/[:.]/g, '-');
const ROOT_DIR = path.resolve('docs/vetus/inspection', `${ROOT_STAMP}-vacinas`);
...
async function collectSummary(page) {
  return await page.evaluate(() => {
    ...
    return {
      ...
      buttons: uniq(Array.from(document.querySelectorAll('button, input[type=button], input[type=submit], [role=button], a')).map(text)).slice(0, 400),
...
async function clickIfVisible(page, selector, name) {
  const targets = [
...
async function main() {
  ...
  await goToUrl(page, LEGACY_VACCINES_URL, credentials);
  await dumpState(page, 'estado-vacinas-legacy');
  await dump(page, 'vacinas-lista');

  for (const action of ['Pesquisar', 'Novo', 'Incluir', 'Vacina', 'Vermífugo']) {
    try {
      const before = page.url();
      const clicked = await clickIfVisible(page, action, action);
      if (!clicked) continue;
      extraActions.push({ action, beforeUrl: before, afterUrl: page.url() });
      await dump(page, `vacinas-${action.toLowerCase().replace(/[^a-z0-9]+/gi, '-')}`);
    } catch {
      // ignore
    }
  }
