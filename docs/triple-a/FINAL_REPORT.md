...
**Code candidate:** `main@83d01e9c60c3027b6068aa1d10a50f7f5c585296` (functional code `59a63087`)
...
Este relatório registra o HEAD documental publicado após as correções de frescor do release gate e acessibilidade clínica. O CI #115 desse SHA terminou com 15/16 jobs: Performance (k6 SLOs) falhou com exit 99/1; o CI #114 do código funcional anterior foi verde 16/16. O resultado não autoriza release; as provas externas, operacionais e humanas exigidas continuam ausentes.
...
O gate estrito local em `83d01e9c` retornou `BLOCKED / NOT PROVEN`, score `34`, critical `23` e `27` P0 abertos, com `publication_allowed=false`; thresholds não foram relaxados.
...
- O CI [#34606095261](https://github.com/ricardoakinaga-dev/cvg-his-v4/actions/runs/34606095261) terminou `failure` no HEAD exato `83d01e9c`: 15/16 jobs passaram; `Run k6 benchmark` terminou exit 99 e `Check SLO results` exit 1. Logs detalhados/metricas do job não estão disponíveis publicamente, então nenhuma causa adicional é inferida.
