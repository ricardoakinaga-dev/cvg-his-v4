...
Observado em `2026-09-12T07:25:20Z`, no candidato de código e documentação
...
| ci_run          | [CI #143](https://github.com/ricardoakinaga-dev/cvg-his-v4/actions/runs/34678150409) é do pai `82ff6eec` e terminou `failure` somente em Performance/k6; [#144](https://github.com/ricardoakinaga-dev/cvg-his-v4/actions/runs/34680215070) é do pai documental `da5dd244` e falhou em `Repository Guards` por checkout raso. Não há ainda CI exata para `56a18736` |
...
| blocked         | CI exata do commit de governança ainda ausente; Performance/k6 falhou no CI #143 do pai, #144 encontrou checkout raso no guard de snapshot, restore/DR real por Docker indisponível; target, attestation, deploy/rollback, UAT, branch governance e autoridade humana permanecem abertos                                                                           |
...
no mesmo SHA; o CI do pai #143 terminou com falha no gate de Performance/k6 e o
CI #144 do pai documental encontrou o checkout raso do guard de snapshot. O
candidato atual corrige esse contrato, mas ainda não há execução exata deste
commit de governança. Por isso este baseline
