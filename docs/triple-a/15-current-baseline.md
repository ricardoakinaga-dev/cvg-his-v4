...
Observado em `2026-09-16T20:35:31Z`, no snapshot documental corrente
`f31cc5329e36efff91c10e9b9b3f4c6e93316046`, sobre o candidato funcional
...
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| current_sha     | `b3b9d38d3b3d0267c1dedcc2e7f9a963a98f3334` (candidato funcional; `f31cc532` é descendente somente documental) |
| main_sha        | `main@f31cc532`; `origin/fix/state-of-art-ci-assurance@fe5406c2` é ancestral, sem commits exclusivos; rollback preservado |
...
| ci_run          | O [CI #191](https://github.com/ricardoakinaga-dev/cvg-his-v4/actions/runs/35144297381) terminou `failure` no snapshot documental `f31cc532`; o Critical Coverage Gate passou. |
| ci_failure      | Coverage falhou por conexão do contrato ML em `:5433`; k6 perdeu 3/9 SLOs; E2E/Visual falharam em 29 snapshots divergentes. Nenhum baseline ou threshold foi alterado. |
...
| verified_remote | Critical Coverage Gate `PASS` no CI #191; o run global permanece `failure` por Coverage, k6, E2E e Visual. Target, recovery, UAT, attestation, governança de branch e autoridade de release seguem ausentes |
| verified_target | `NOT_PROVEN` |
| blocked         | Run global não verde, target, restore/DR, performance certificada, UAT e autoridade humana continuam abertos; os thresholds gerais e snapshots permanecem sem promoção |
| not_proven      | Qualquer claim de release Triple-A, score ≥97, critical ≥95, zero P0, `main_green` ou `TRIPLE-A VERIFIED` |

## Decisão

O candidato funcional `b3b9d38d3b3d0267c1dedcc2e7f9a963a98f3334` foi reconciliado no
snapshot documental `f31cc532`;
...
recoleta crítica/Vue passou no Critical Coverage Gate do CI #191, mas o run global
terminou com falhas em Coverage, k6, E2E e Visual. A `main` permanece bloqueada para Green Main
