...
`aa6ab73e70bd8f97b1b61c2e0eccb2380545117c`, que contém a identidade corrente
...
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| current_sha     | `aa6ab73e70bd8f97b1b61c2e0eccb2380545117c` (snapshot; registro P0 e manifesto revision 70; Noto Sans e rasterização sem LCD determinísticos nos gates E2E/visual; verificação portátil do runner; endurecimento funcional do provider de feature flags, autoridade request-scoped, ownership composto, classificador documental e ciclo de vida; harness SPA canônico com proxy same-origin; `server.ts` em `8.335` linhas) |
| main_sha        | `main` local contém o candidato `fc3230b3`; a `main` remota está publicada em `origin/main@aa6ab73e`; `origin/fix/state-of-art-ci-assurance@fe5406c2` é ancestral, sem commits funcionais exclusivos; rollback preservado |
...
| ci_run          | O [CI #235](https://github.com/ricardoakinaga-dev/cvg-his-v4/actions/runs/35244228724) falhou no Repository Guards porque `server.ts` excedia o limite congelado (`8.384 > 8.335`); o resultado não é promovido. |
...
o manifesto crítico foi reancorado na revisão 70, com a correção de tipografia e rasterização CI/visual, a verificação portátil do runner, o código funcional de feature flags e os
