...
> [`CURRENT_CANDIDATE_IDENTITY.json`](./CURRENT_CANDIDATE_IDENTITY.json). O candidato corrente é `aa6ab73e70bd8f97b1b61c2e0eccb2380545117c`; o registro P0 candidate-bound é validado no CI e no gate de release; Noto Sans e a rasterização sem LCD são fixados nos gates E2E/visual; o endurecimento funcional de fail-closed, tenant explícito, cache bounded, upsert atômico, autoridade request-scoped e ownership composto está em `03e6fac0`; o orçamento de `server.ts` está em `8.335` linhas; os CI anteriores não são transferidos. O runner crítico usa PostgreSQL 16, checkout completo, trata o par V8 do Node 22 e publica evidência SQL; nenhum resultado parcial é transferido.
> Nenhum threshold foi relaxado e o veredito geral continua `BLOCKED / NOT PROVEN`.

**Current snapshot:** `aa6ab73e70bd8f97b1b61c2e0eccb2380545117c` (snapshot reancorado; manifesto crítico revision 70, registro P0 com `1` fechado e `13` abertos, workflow com Chromium, Noto Sans e rasterização sem LCD determinísticos nos gates E2E/visual, verificação portátil do runner, runner privado PostgreSQL 16, par V8 autenticado, produtor SQL, endurecimento fail-closed, tenant explícito, cache bounded/invalidação, upsert atômico, autoridade request-scoped, ownership composto, harness SPA com proxy same-origin e `server.ts` dentro do orçamento de `8.335` linhas; provas externas e autoridade de release continuam `NOT PROVEN`).
