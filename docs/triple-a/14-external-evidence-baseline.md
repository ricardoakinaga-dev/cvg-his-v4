...
> [`CURRENT_CANDIDATE_IDENTITY.json`](./CURRENT_CANDIDATE_IDENTITY.json). O candidato corrente é `271bc3c3658a1a0e31075ed96f74f07f75decf44`; o registro P0 candidate-bound é validado no CI e no gate de release; Noto Sans e a rasterização sem LCD são fixados nos gates E2E/visual; o orçamento de `server.ts` está em `8.335` linhas; a correção do contrato de evidência SQL legada está incluída; os CI anteriores não são transferidos. O runner crítico usa PostgreSQL 16, checkout completo, trata o par V8 do Node 22 e publica evidência SQL; nenhum resultado parcial é transferido.
> Nenhum threshold foi relaxado e o veredito geral continua `BLOCKED / NOT PROVEN`.

**Current snapshot:** `271bc3c3658a1a0e31075ed96f74f07f75decf44` (snapshot reancorado; manifesto crítico revision 73, registro P0 com `1` fechado e `13` abertos, workflow com Chromium, Noto Sans e rasterização sem LCD determinísticos nos gates E2E/visual, verificação portátil do runner, runner privado PostgreSQL 16, par V8 autenticado, produtor SQL com contrato legado limitado a 0170/0171/0172 e execução de 0173/0174 para validar o schema final, cobertura de UUIDs nas rotas, harness SPA com proxy same-origin, `server.ts` dentro do orçamento de `8.335` linhas e cobertura global local em `82,00%` branches; provas externas e autoridade de release continuam `NOT PROVEN`).
**Current CI:** a execução exata do candidato otimizado será coletada após esta reancoragem; nenhum resultado histórico é transferido e nenhum threshold foi relaxado.
**Current status:** **BLOCKED / NOT PROVEN**
**Observation:** 2026-09-18T01:10:00Z
