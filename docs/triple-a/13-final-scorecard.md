...
| Campo              | Estado                                                                                                                                                                                                                                                    |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CURRENT SNAPSHOT   | candidato `5a5388281633a235a6e53909c7432c4535194d70`; identidade canônica em `CURRENT_CANDIDATE_IDENTITY.json`; registro P0 candidate-bound; importador do evidence graph com validação de envelope, frescor, ancestralidade, ambiente e digest; nenhum envelope local é promovido a PASS; históricos não são transferidos |
| MAIN / ORIGIN      | a `main` remota estava publicada via Git Data API sem force-push; este candidato acrescenta o importador do evidence graph e será publicado sem force-push; `origin/fix/state-of-art-ci-assurance@fe5406c2` permanece ancestral sem commits funcionais exclusivos |
...
| LOCAL VALIDATION   | Neste candidato, typecheck e lint do workspace passaram em `68/68` projetos; module-feature-flags passou `10/10` testes, os testes focados da API passaram `19/19`, a suíte crítica corrente passou `67/67` arquivos/`618/618` testes de integração e `11/11` cenários de processo, e identidade, registro P0, importador candidate-bound e complexity de `8.335` linhas foram validados. SQL `34/34`, visual `29/29` e k6 local `9/9` permanecem evidências locais/históricas anteriores; target externo ausente |
