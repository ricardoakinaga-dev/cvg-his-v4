...
| Campo              | Estado                                                                                                                                                                                                                                                    |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CURRENT SNAPSHOT   | candidato `0fab9fc189e122b4ff7e7700d9b07508e64f54bd`; identidade canônica em `CURRENT_CANDIDATE_IDENTITY.json`; registro P0 candidate-bound; importador do evidence graph com validação de envelope, frescor, ancestralidade, ambiente e digest; nenhum envelope local é promovido a PASS; históricos não são transferidos |
| MAIN / ORIGIN      | `main` remota publicada em `8bbcba3a668709d1ade4588ac59f56948d1b420d`; este candidato funcional está pronto para publicação sem force-push; `origin/fix/state-of-art-ci-assurance@fe5406c2` e `origin/codex/state-of-art-hardening-20260917@a902fc24` permanecem ancestrais já integradas |
| CURRENT CI         | O [CI #251](https://github.com/ricardoakinaga-dev/cvg-his-v4/actions/runs/35281221617) pertence ao predecessor documental `8bbcba3a`; não é transferido para este candidato, que aguarda execução exata após a publicação |
...
| LOCAL VALIDATION   | Neste candidato, typecheck e build completos passaram em `68/68` projetos; a suíte completa passou `394/394` arquivos e `3932/3932` testes com as `176` migrações aplicadas em PostgreSQL efêmero; access-control passou `48/48` e migration-source, RLS `171/172`, namespaces, OpenAPI, docs, identidade, P0 e complexity passaram. A validação local não prova CI exato, target, recovery, UAT, attestation ou autoridade |
