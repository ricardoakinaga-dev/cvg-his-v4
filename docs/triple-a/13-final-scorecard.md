...
| Campo              | Estado                                                                                                                                                                                                                                                    |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CURRENT SNAPSHOT   | candidato `aa6ab73e70bd8f97b1b61c2e0eccb2380545117c`; identidade canônica em `CURRENT_CANDIDATE_IDENTITY.json`; registro P0 candidate-bound; Noto Sans e rasterização de texto determinísticos nos gates E2E/visual; orçamento de `server.ts` em `8.335` linhas; históricos não são transferidos |
| MAIN / ORIGIN      | `main` local contém o candidato `aa6ab73e`; a `main` remota está publicada via Git Data API sem force-push; `origin/fix/state-of-art-ci-assurance@fe5406c2` é ancestral sem commits funcionais exclusivos, rollback preservado |
| CURRENT CI         | O [CI #235](https://github.com/ricardoakinaga-dev/cvg-his-v4/actions/runs/35244228724), executado no candidato remoto anterior, falhou no Repository Guards porque `server.ts` excedia o limite congelado (`8.384 > 8.335`); o candidato corrigido será avaliado por um novo CI exato |
...
| LOCAL VALIDATION   | Neste candidato, typecheck e lint do workspace passaram em `68/68` projetos; module-feature-flags passou `10/10` testes e os testes focados da API/repositório passaram `17/17`; manifesto revision 70, identidade, registro P0 e complexity de `8.335` linhas foram validados. A cobertura global `273/273`, API `618/618`, integração PostgreSQL `16/16`, SQL `34/34`, visual `29/29` e k6 local `9/9` permanecem evidências locais/históricas anteriores; target externo ausente |
...
na revisão 70. O registro P0 adiciona deduplicação, dependências, classificação
