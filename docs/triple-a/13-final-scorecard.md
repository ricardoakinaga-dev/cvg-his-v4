...
| Campo              | Estado                                                                                                                                                                                                                                                    |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CURRENT SNAPSHOT   | candidato `5998287d386559fc3b3ed760a47756e62349f286`; identidade canônica em `CURRENT_CANDIDATE_IDENTITY.json`; registro P0 candidate-bound; importador do evidence graph com validação de envelope, frescor, ancestralidade, ambiente e digest; nenhum envelope local é promovido a PASS; históricos não são transferidos |
| MAIN / ORIGIN      | `main` remota publicada em `3b4e24f3804d79190b4ea24e594930df5ced9b7f`; este candidato funcional está pronto para publicação sem force-push; `origin/fix/state-of-art-ci-assurance@fe5406c2` e `origin/codex/state-of-art-hardening-20260917@a902fc24` permanecem ancestrais já integradas |
| CURRENT CI         | O [CI #247](https://github.com/ricardoakinaga-dev/cvg-his-v4/actions/runs/35276834295) do predecessor documental `3b4e24f3` passou os gates funcionais, E2E SPA, Visual e segurança, mas falhou em Critical Coverage e Performance; não é transferido para este candidato, que aguarda execução exata após a publicação |
...
| LOCAL VALIDATION   | Neste candidato, typecheck e build completos passaram em `68/68` projetos; a suíte real de access-control passou `48/48` testes com as `175` migrações aplicadas em PostgreSQL efêmero; migration-source, RLS `171/172`, namespaces, OpenAPI, docs, identidade, P0 e complexity passaram. A validação local não prova CI exato, target, recovery, UAT, attestation ou autoridade |
| VERIFIED TARGET    | `NOT PROVEN`                                                                                                                                                                                                                                              |
| CURRENT VERDICT    | **BLOCKED / NOT PROVEN**; o CI exato deste candidato ainda não foi executado, o predecessor falhou em Critical Coverage/Performance e target, recovery, UAT e autoridade externa continuam sem prova |

O snapshot corrente preserva a régua congelada e reancora o manifesto crítico
na revisão 74. O registro P0 adiciona deduplicação, dependências, classificação
