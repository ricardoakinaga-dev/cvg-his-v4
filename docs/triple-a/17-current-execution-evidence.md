...
## Candidato de código observado em 2026-09-16T02:05:04Z

- SHA de código e documentação: `371e5f844e984b26acb2dc2e1302031ca2a94409`; identidade em
...
- `HEAD`/`main` local está em `371e5f84`; `origin/main` está em `e4e18fba`; rollback preservado em `origin/fix/state-of-art-ci-assurance@fe5406c2`.
...
| Escopo                 | Resultado                                                                                                                                                                                                                                                                                                                                                                     |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gate estrito           | `BLOCKED`, score `52`, critical `51`, open P0 `17`, `claim=NOT PROVEN`, `publication_allowed=false`, executado explicitamente com `commitSha=371e5f84` |
...
Não existe ainda run remoto vinculado ao SHA exato `371e5f84`; [CI #35046394357](https://github.com/ricardoakinaga-dev/cvg-his-v4/actions/runs/35046394357) é do candidato anterior `e4e18fba` e falhou na provisão APT; permanece somente diagnóstico histórico. O gate local explícito do candidato retornou `52/51/17` e bloqueou publicação. A publicação do candidato deve disparar nova execução com os guards vinculados à identidade corrente.
...
`371e5f84` é `BLOCKED / NOT PROVEN` (`52/51/17`); a execução completa passou
