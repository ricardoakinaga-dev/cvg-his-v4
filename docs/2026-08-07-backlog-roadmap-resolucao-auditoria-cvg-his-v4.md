...
| AUD-010 | Migrar tokens para sessão segura | `SEC/WEB` | 13 | - | access token não fica em `localStorage`; refresh em cookie `HttpOnly/Secure/SameSite`; rotação, revogação e CSRF testados | verificado |
...
| AUD-012 | Rate limit distribuído e proxy confiável | `SEC/PLAT` | 8 | - | Redis é usado em produção; `X-Forwarded-For` só é aceito de proxies configurados; chave inclui rota, conta e origem apropriadas | verificado |
...
**Saída O0 atualizada:** `AUD-001` a `AUD-012` verificados; `AUD-013` e `AUD-014` parciais. Os gates técnicos de cobertura, API, worker, PostgreSQL crítico, OpenAPI, RLS e segurança estão verdes; o núcleo de runtime DB e o E2E de dois tenants foram comprovados. G1 continua condicional à expansão da matriz para todos os agregados e providers externos.
...
- `AUD-010` a `AUD-014`, `PLAT-001` a `PLAT-003`, `SEC-001/002` e `API-001` verificados;
...
|---|---|---|
| P0 `AUD-001`–`AUD-012` | verificados | código, testes API/worker, cobertura e gates completos |
...
| Restore drill | checksums, globals, dump, storage e diff vazio; 2 tabelas e 2 arquivos | `ops:restore:drill:fixture`, evidência em `/tmp/cvg-his-v2-restore-drill-evidence-final` |
...
|---|---|---|---|
| `AUD-001`–`AUD-012` | verificado; `AUD-013/014` parciais | API 263/263, coverage 1.427 testes, critical 230/230, E2E DB e security green | expansão da cobertura de runtime, matriz completa de providers e CI externo |
