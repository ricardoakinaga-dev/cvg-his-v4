...
| Estoque > Configuracoes Fiscais > Tabela PIS | Cadastro fiscal de aliquotas PIS por codigo, descricao e percentual | Observado; ajuste pendente | Pendente | Observacao direta atual em 2026-04-28 bloqueada por indisponibilidade da ponte Playwright/Chrome antes do login; sem escrita no Vetus. Mapeamento confirmado por artefatos read-only autenticados de 2026-04-23: rota Vetus `/pis`, breadcrumb `Estoque > Configuracoes Fiscais > Tabela PIS`, titulo `Tabela PIS`, apoio `Quer cadastrar PIS de forma pratica? Saiba Mais`, busca `Buscar por codigo ou descricao`, botao `Incluir Nova Tabela`, estado vazio `Nenhum registro cadastrado`, backend `GET /tax/pis`, modelo `tax_pis` com `code`, `description`, `percent`. No `cvg-his-v2`, `/fiscal/pis` existe, mas reaproveita a tela conjunta `PIS / COFINS`, parcial/read-only e focada em regras por regime/aplicacao; proximo ajuste deve espelhar a tela Vetus como cadastro simples de PIS, sem misturar com consulta conjunta PIS/COFINS. |
...
- SPA: `http://localhost:3002`
- API: `http://localhost:3003`
- Ultima rota validada: `http://localhost:3002/medical-records/enc-1`
- API health validado em `http://localhost:3003/health`
- Ultima API validada: contrato SPA/API para `GET /medical-records?encounterId=:id`, `GET /medical-records/entries?encounterId=:id`, `POST /medical-records/entries`, `GET /encounters/:id`, `GET /patients/:id`, `GET /owners/:id`, `GET /billing/:encounterId`, `GET /billing/:encounterId/items`
...
| Estoque | Em andamento | Implementar ajuste de paridade de `Estoque > Configuracoes Fiscais > Tabela PIS` conforme mapeamento observacional de 2026-04-28. |
